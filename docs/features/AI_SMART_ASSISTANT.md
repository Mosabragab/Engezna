...
## ⚠️ ملاحظة مهمة للإدارة - الميزة معطلة مؤقتاً

| البند | التفاصيل |
|-------|----------|
| **الحالة الحالية** | 🔴 **معطل مؤقتاً** |
| **سبب التعطيل** | النظام لا يزال في مرحلة التطوير وغير محكم بشكل كافٍ للإطلاق |
| **الهدف** | تقليل المشاكل في الفترات الأولى من الإطلاق |
| **تاريخ التعطيل** | يناير 2026 |
| **الخطة المستقبلية** | سيتم تفعيل الميزة بعد إجراء المزيد من الاختبارات والتحسينات |

### كيفية تفعيل الميزة مستقبلاً

لتفعيل المساعد الذكي، قم بإضافة المتغير التالي في ملف `.env`:

```env
NEXT_PUBLIC_AI_ASSISTANT_ENABLED=true
```

**ملاحظة:** القيمة الافتراضية هي `false` (معطل)، لذا إذا لم يكن المتغير موجوداً، سيبقى المساعد مخفياً.
