...
> آخر تحديث: 2026-05-12 — Tasks A/B/C/B-bis/C-bis/D/E/F ✅ merged. **Artifact #24 (post Task F)**: median perf — auth/login 0.95، cart 0.92، provider/login 0.92، welcome 0.82، /ar 0.84، custom-order 0.83، providers 0.79، providers/<id> 0.77. Task G قيد التنفيذ على `claude/perf-finish-routes`: 3 fixes consolidated — (1) Codex fix على Task F priority (card 0 only، not 0+1)، (2) banner shine animation finite (block LCP على providers/<id>)، (3) arefRuqaa `display: optional` (يلغي LCP swap re-evaluation على custom-order/auth). متوقع تحسين على /ar/providers/<id> و /ar/custom-order و /ar/providers معاً. render-blocking warn deferred.
