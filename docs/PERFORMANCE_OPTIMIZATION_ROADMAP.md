...
> آخر تحديث: 2026-05-12 — Tasks A/B/C/B-bis/C-bis ✅ merged. C-ter REVERTED. Task D قيد التنفيذ على PR #387 (5 commits): outer skeleton + spinner branch + BannerCard motion.div + web splash + OffersCarousel skeleton DOM aligned with real (section.children[1] = div.relative في الحالتين). الـ root cause الحقيقي اتحدد في الـ commit الأخير: Lighthouse `layout-shifts` يـ match elements بـ DOM path index لا class names. render-blocking warn يبقى deferred لـ structural fix لاحقاً.
