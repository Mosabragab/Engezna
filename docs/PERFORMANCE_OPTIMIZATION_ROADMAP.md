...
> فرع التنفيذ الحالي للأداء: `claude/perf-puppeteer-home-setup` (Task B-bis — infrastructure ready، waits on user-supplied real governorate/city UUIDs). C-ter (PR #385) merged مع flag غلط (`optimizeCss` بدل `inlineCss`)؛ التصحيح في `claude/perf-css-render-blocking` follow-up.
> آخر تحديث: 2026-05-11 — Tasks A/B/C/C-bis/C-ter ✅ merged. B-bis في الـ PR الحالي. C-ter يحتاج follow-up لتصحيح الـ flag بعد Codex catch (App Router يستخدم `inlineCss` مش `optimizeCss`).
...
| **B-bis**     | Infrastructure: puppeteer devDep + lhci-home-setup.js seeds localStorage + workflow Chromium cache + env-driven seed gate + UUID validation. الـ home measurement يحتاج user يضبط real UUIDs في GitHub Secrets | `claude/perf-puppeteer-home-setup`          | 🔄 الـ PR الحالي (infra ready)              | B merged ✓ + user-supplied real UUIDs |
...
| **C-ter**     | enable `experimental.optimizeCss: true` + install `critters` devDep (Codex: الـ flag غلط للـ App Router؛ inlineCss هو الصحيح — تصحيح في follow-up branch)                                                      | `claude/perf-css-render-blocking`           | ✅ merged PR #385 (يحتاج follow-up)         | C-bis merged ✓                        |
| **C-ter-fix** | تصحيح C-ter: `experimental.inlineCss: true` (App Router) + حذف `critters` devDep                                                                                                                               | `claude/perf-css-render-blocking` (cb18992) | 🔄 follow-up PR                             | C-ter merged ✓                        |
