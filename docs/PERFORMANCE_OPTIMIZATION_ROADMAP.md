...
> فرع التنفيذ الحالي للأداء: `claude/perf-css-render-blocking` (Task C-ter — `experimental.inlineCss` للـ App Router. تصحيح بعد Codex catch: optimizeCss كان Pages Router فقط).
...
| **C-ter** | enable `experimental.inlineCss: true` (App Router built-in، لا dependency) — inline critical CSS في initial HTML، defer rest. يستهدف render-blocking warn على 3 routes | `claude/perf-css-render-blocking`     | 🔄 الـ PR الحالي                            | C-bis merged ✓                           |
