...
| ------ | -------------------------------------- | ----------- | ------- | -------- | -------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **P0** | `/ar` (الـ home الفعلي بعد location)   | **47 POOR** | 44      | **2332** | يُقاس في B-bis | 🔄 جاهز لـ D بعد artifact | كان blocked على cookie + localStorage gates. cookie في extraHeaders + Puppeteer script يـ seed localStorage عبر `scripts/lhci-home-setup.js` (الـ PR الحالي) |
...
- **`/ar` redirect — ✅ مغلق في task B-bis (الـ PR الحالي):** الـ home gated بـ middleware cookie + client-side localStorage check. `extraHeaders` يحقن الكوكي، و `scripts/lhci-home-setup.js` (puppeteer setup) يـ seed `engezna_guest_location` في localStorage قبل كل lighthouse audit. `/ar` رجع للـ URL list في `lighthouserc.js` بأمان. أول CI run بعد merge سيكون عنده home measurements (P0).
