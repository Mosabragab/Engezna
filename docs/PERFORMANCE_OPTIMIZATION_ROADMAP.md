...
> آخر تحديث: 2026-05-12 — Tasks A/B/C/B-bis/C-bis ✅ merged. C-ter REVERTED (مفيش flag مناسب للـ App Router). Task D (P0 home CLS) قيد التنفيذ على PR #387: outer skeleton + spinner branch + BannerCard motion.div + web splash (NativeSplashHider) كلها أُزيلت تباعاً. render-blocking warn يبقى deferred لـ structural fix لاحقاً.
...
| **D**     | P0 home CLS: حذف الـ outer skeleton block في `HomePageClient.tsx` (الـ `if (isLoading) return` القديم) + حذف الـ `!userLocation.governorateId` full-screen spinner branch (Codex catch — كان يعيد إنتاج CLS regression خلال hydration window) + تحديث الـ wrapper reservations لـ `min-h-[320px] md:min-h-[280px]` على offers و `min-h-[320px] sm:min-h-[200px]` على categories. السبب: artifact CI #230 كشف CLS = 0.463 على `/ar`، الـ culprits بالـ score الفعلي = OffersCarousel banner 0.319 + CategoriesSection 0.140 (+ 0.002 ثانوي). الـ outer skeleton DOM وبنية الـ spinner مختلفان عن الـ real components — anti-pattern مزدوج. التفاصيل في §٧ "بعد تفعيل seeds + B-bis (2026-05-12)" | `claude/perf-home-cls`                | PR                                          | B-bis merged ✓ + real seed secrets set ✓ |
...
- **اكتشاف 2026-05-12 (web splash overlay — Task D part 4):** بعد commit 3 (BannerCard motion.div → div) الـ CLS لسه ثابت عند 0.242 بدون أي تحسين، إشارة إن الـ hypothesis كانت غلط. فحص الـ filmstrip في artifact #18 كشف الـ root cause الحقيقي: `NativeSplashHider` (src/components/shared/NativeSplashHider.tsx) كان يـ render `fixed inset-0` overlay لمدة 2.5s على web (once per sessionStorage). thumb 2 (3599ms) = splash logo + tagline، thumb 3 (4799ms) = real home. الـ CLS كان يقيس النافذة اللي بين الـ splash unmount-fade والـ home sections settling في الـ DOM tree خلف الـ overlay، فالـ section.children[1] (banner container) كان يـ register كـ shift كبير. الـ splash مكتوب أصلاً للـ native (Capacitor) عشان يـ soften الـ web content loading، لكنه على web يضيف 2.5s قبل الـ user يشوف الـ home + يلوّث CI metrics. **عُولج (commit 4):** الـ web path في `NativeSplashHider` يرجع `false` دلوقتي — الـ splash يـ fire للـ native بس. SSR HTML يبقى المرئي مباشرة على web، اللي هو الـ path اللي lab بيقيسه فعلاً. الـ session-shown polish UX يفقد لكن metrics ≫ UX polish per §1 priorities.
