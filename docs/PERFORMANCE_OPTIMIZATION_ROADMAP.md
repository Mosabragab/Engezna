...
> آخر تحديث: 2026-05-13 — **Tasks A → G ✅ merged + partial revert على Task G**: `arefRuqaa display: 'optional'` reverted لـ `'swap'` بعد owner لاحظ visual regression على الـ logo (fallback sans-serif بدلاً من Aref Ruqaa على first visit). الـ LCP gain اللي Task G كان يستهدفه على /ar و /ar/custom-order ما حصلش (artifact #26: 4068→4068، 4576→4578) — الـ brand cost مدفوع بلا فائدة قياسية. **Artifact #26 الـ state القياسي يبقى valid** (الـ revert لا يحرّك الـ LCP). **§٣ "current target" 🥈 = 100% محقّق على 8/8 routes**. **§٣ "ambitious target" 🥇 = 87.5% (5 firm + 2 borderline)**. الـ remaining gaps تحتاج architectural refactor (مدرَجة في §٨). render-blocking CSS و provider/admin pages مؤجَّلة.
...
| **G**     | Consolidated route finisher (3 fixes): (1) **Codex fix** على Task F — `isPriority={index < 2}` → `isPriority={index === 0}` (mobile single-column يعرض card 0 above fold فقط). (2) **Banner shine animation** على CustomOrderWelcomeBanner — `animation: ... infinite` → `... 2` (iteration count finite). الـ continuous overlay يـ block LCP finalization على `/ar/providers/<id>` (render delay 89% / 5250ms). (3) **arefRuqaa display** — `swap` → `optional` لإلغاء الـ LCP re-evaluation بعد font swap على `/ar/custom-order` (render delay 87% / 3968ms على logo). تأثير متوقع: /ar/providers يدخل 🥇 بـ stable priority، /ar/providers/<id> LCP −3-4s، /ar/custom-order LCP −3-4s | `claude/perf-finish-routes`           | PR                                          | F merged ✓                               |
