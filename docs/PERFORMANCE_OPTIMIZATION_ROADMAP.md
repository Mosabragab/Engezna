...
> آخر تحديث: 2026-05-13 — **Tasks A → G ✅ merged**. **Task H قيد التنفيذ (logo → SVG)**: owner اقترح تثبيت الـ logo كـ SVG asset بدلاً من text + font لتجنّب الـ swap اللحظي. تم توليد `public/logos/engezna-mark.svg` (3.9kb) من vector outlines الـ Aref Ruqaa woff2 + استبدال كل usages (EngeznaLogo، offline page، pwa.css splash) + حذف الـ arefRuqaa font import + الـ woff2 (45kb saved). الـ logo دلوقتي paints مع FCP بدون font dependency، no swap، brand fidelity 100%. متوقع: `/ar` و `/ar/custom-order` يخرجوا من borderline ويدخلوا 🥇 firm (الـ font-swap LCP wobble eliminated). render-blocking CSS و provider/admin pages مؤجَّلة.
...
| **F**     | `/ar/providers` LCP image priority: إضافة `isPriority` prop على ProviderCard، الـ ProvidersClient يمرّر `isPriority={index < 2}`. **NOTE:** artifact #24 أكّد التحسّن variable (run واحد 0.88، median 0.79 لسه). Codex catch post-merge: على mobile single-column card 1 below-fold فالـ priority بيتنافس على bandwidth مع الـ actual LCP image — مدرَج في Task G                                                                                                                                                                                                                                                                                         | `claude/perf-provider-card-lcp`       | ✅ merged PR #390                           | E merged ✓                               |
| **G**     | Consolidated route finisher (3 fixes): (1) **Codex fix** على Task F — `isPriority={index < 2}` → `isPriority={index === 0}` (mobile single-column يعرض card 0 above fold فقط). (2) **Banner shine animation** على CustomOrderWelcomeBanner — `animation: ... infinite` → `... 2 forwards` (iteration count finite + CodeRabbit forwards). (3) **arefRuqaa display** — `swap` → `optional` (REVERTED بعد الـ merge لـ brand fidelity rejection — انظر §٨)                                                                                                                                                                                                  | `claude/perf-finish-routes`           | ✅ merged PR #391                           | F merged ✓                               |
| **H**     | Logo as SVG asset (owner proposal): استبدال الـ "إنجزنا" text + arefRuqaa font بـ pre-rendered SVG (vector outlines من الـ font glyphs). الـ SVG paints مع FCP بدون font dependency، no swap window، no LCP re-evaluation. تم توليد `public/logos/engezna-mark.svg` (3.9kb) من الـ woff2 outlines عبر fontTools + uharfbuzz shaping. استبدال في 3 usages: `EngeznaLogo.tsx` (static + animated)، `offline/page.tsx`، `pwa.css`. حذف الـ arefRuqaa font import + الـ 45kb woff2. تأثير متوقع: /ar و /ar/custom-order يخرجوا من borderline 🥇 لـ firm 🥇 (font-swap LCP wobble eliminated). brand fidelity 100% لأن الـ vector outlines are the font glyphs | `claude/perf-logo-svg`                | PR                                          | G merged ✓                               |
...
- ✅ **5 routes في 🥇 كاملة** (كان 2 قبل Task E، 4 بعد Task E): /ar/auth/login، /ar/cart، /ar/welcome، /ar/provider/login + **/ar/providers ✨** (Task F priority cards دفعه).
- ✅ **CI assertions تمرّ بدون errors** على artifacts #20-25 المتتالية.
...
### بعد Task F merged (2026-05-13) — artifact #25 baseline قبل Task G

artifact: `lighthouseresults_25.zip` (CI run بعد دمج Task F (priority cards) على main). 7 URLs × 3 runs. الـ table بيـ measure الـ state اللي Task G مفروض يعالجه.

| URL                  | Perf (median) |    LCP |   TBT |    TTI | Tier                          |
| -------------------- | ------------: | -----: | ----: | -----: | ----------------------------- |
| `/ar/auth/login`     |          0.96 | 2567ms | 101ms | 3624ms | 🥇                            |
| `/ar/cart`           |          0.96 | 2565ms | 129ms | 4936ms | 🥇                            |
| `/ar/providers`      |          0.84 | 3993ms | 201ms | 4000ms | 🥇 ✨ (just under thresholds) |
| `/ar/welcome`        |          0.83 | 2585ms | 380ms | 5030ms | 🥇                            |
| `/ar/provider/login` |          0.91 | 3013ms | 197ms | 5305ms | 🥇                            |
| `/ar/custom-order`   |          0.83 | 4576ms | 132ms | 4576ms | borderline 🥇 (LCP +576ms)    |
| `/ar`                |          0.81 | 4068ms | 253ms | 5729ms | borderline 🥇 (LCP +68ms)     |
| `/ar/providers/<id>` |          0.75 | 5942ms | 150ms | 5949ms | 🥈                            |

**5/8 firm 🥇 + 2 borderline + 1 🥈**. الـ /ar/providers دخل 🥇 لأول مرة (Task F priority cards worked) لكن variable بين runs.

**الـ 3 routes الباقية المشاكل:**
...
- **Task H 2026-05-13 (logo SVG — يحلّ الـ swap window من جذره):** بعد revert الـ arefRuqaa `display: optional`، owner اقترح السؤال الأهم: لماذا الـ logo هو text + font أصلاً وليس asset مثبَّت؟ الـ font approach يضمن swap window موجود دائماً مهما تكن قيمة `font-display`. الـ proper fix architectural: استبدال الـ text بـ SVG vector asset مولَّد من نفس الـ font outlines. **التنفيذ:** (1) Python script بـ fontTools + uharfbuzz: load `aref-ruqaa-700.woff2`، shape "إنجزنا" بـ HarfBuzz للحصول على الـ contextual Arabic forms (initial/medial/final/isolated)، استخراج الـ SVG path commands لكل glyph، composite في SVG واحد مع viewBox + scale flip (font y-up → SVG y-down). الناتج 3.9kb (مقابل 45kb woff2)، 11 shaped glyphs (6 letters + 5 dots/marks). الـ visual رُوجِع بـ cairosvg render إلى PNG — matches Aref Ruqaa exactly. (2) `EngeznaLogo.tsx` — static branch بقى `<img src="/logos/engezna-mark.svg">` بدلاً من styled `<span>`. animated branch نفس الـ pattern مع reveal overlay كـ sibling div (لأن `<img>` لا يدعم `::after` pseudo-elements). (3) `offline/page.tsx` و `pwa.css` — switch لـ `<img>` و `mask-image` respectively. (4) `lib/fonts/index.ts` و `layout.tsx` — حذف `arefRuqaa` export + variable className. (5) `public/fonts/aref-ruqaa-700.woff2` — deleted (45kb saved من الـ deploy). **الـ branding folder** (`project-management/marketing/branding/logo-kit/`) راجعتها — فيها designer tools (logo-exporter.html، engezna-logo-ruqaa.html) لكن مفيش brand-approved SVG asset جاهز. الـ `exportSVG()` في logo-exporter يـ embed نص + Google Fonts CSS (مش outlined vectors). فالـ SVG المولَّد من الـ woff2 outlines هو الـ proper approach. **النتيجة المتوقعة:** الـ logo paints فوراً عند FCP، no font request، no swap، no LCP re-evaluation. على routes الـ logo هو الـ LCP element (/ar، /ar/custom-order)، الـ render delay 84-87% المرصود في artifacts #25-26 يجب أن يـ collapse لـ render delay طبيعي (~10-15%). متوقع entry لـ firm 🥇 لـ /ar (كان borderline +68ms) و دخول قريب لـ /ar/custom-order (كان borderline +578ms). الـ /ar/providers/<id> ما هيتأثر بـ Task H (الـ LCP هناك banner text، مش logo).

- **arefRuqaa display:optional REVERTED 2026-05-13 (brand fidelity rejection):** بعد Task G merged + artifact #26 صدر، owner شاف الـ logo على `/ar` يظهر بـ plain sans-serif fallback بدلاً من Aref Ruqaa calligraphic font. الـ refresh أعاد الـ Aref Ruqaa (cache hit) — أكّد إن الـ `display: 'optional'` يـ behave زي ما documented: block 100ms، لو الـ font ما جاش يـ lock الـ fallback لـ rest of page load. على cold connection (first visit، no font cache) → fallback يظهر دائماً. **الـ trade-off اللي Task G commit message وثّقه** اتفّر بالـ user رفضه: الـ brand identity (الـ "إنجزنا" logo بـ Aref Ruqaa calligraphic) جزء جوهري من الـ visual brand، مش degradation acceptable. الـ artifact #26 data أكّد إن الـ LCP gain اللي قصدنا تحقّقه ما حصلش: `/ar` 4068→4068، `/ar/custom-order` 4576→4578 — basically zero change. الـ brand cost كان مدفوع بدون فائدة قياسية. **عُولج**: revert `display: 'optional'` → `'swap'` في `src/lib/fonts/index.ts`. الـ logo دلوقتي يـ paint في fallback مؤقتاً (FCP)، يـ swap لـ Aref Ruqaa لما يصل (within 1-2s على fresh connection). الـ tier matrix من artifact #26 يبقى valid لأن الـ LCP لم يتأثر إيجاباً بالـ optional أصلاً. **الدرس العام**: brand identity elements لا تخضع لـ font-display: optional قرارات. الـ proper fix لـ font swap LCP penalty يبقى scoped preload (architectural، deferred) أو أن نقبل الـ borderline 🥇 على الـ routes اللي اللوجو فيها LCP.

- **CodeRabbit follow-up على Task G 2026-05-13 (animation-fill-mode forwards):** بعد Task G PR (commit 573b08c) فُتح للـ review، CodeRabbit حذّر إن الـ `.animate-banner-shine` بـ `animation: banner-shine 3s ease-in-out 2` (بدون `forwards`) يـ revert الـ overlay لـ initial transform بعد الـ 2 iterations. الـ initial state هو `translateX(-100%)` في الـ @keyframes لكن CSS animations بدون `fill-mode` بـ default ترجع لـ COMPUTED VALUE قبل الـ animation — اللي هو `transform: none` (= `translateX(0)`). فالـ overlay يـ animate twice ثم يستقر فوق الـ banner ON TOP of LCP element. الـ situation أسوأ من قبل: بدلاً من overlay متحرك (predictable noise)، عندنا stationary overlay في الـ center. CodeRabbit الـ fix الصحيح: إضافة `animation-fill-mode: forwards` (أو `forwards` في shorthand) فالـ overlay يستقر في الـ final keyframe (`translateX(100%)`) — off-screen right، out of view من الـ LCP element. **عُولج في commit متابع على نفس الـ branch:** `animation: banner-shine 3s ease-in-out 2 forwards`. الـ change مهم لـ /ar/providers/<id> اللي LCP عنده 5942ms مع render delay 90% — لو الـ overlay يستقر off-screen بعد الـ 2 iterations (6s)، Lighthouse يقدر finalize LCP بمجرد ما الـ banner text يستقر.

- **Task G 2026-05-12 (Consolidated finisher — 3 fixes في PR واحد):** بعد Task F merged، artifact #24 أكّد إن `/ar/providers` بقى variable (run 0.88، median 0.79) — الـ priority ساعد لكن الـ median لسه تحت 0.80. **Codex راجع الـ Task F المدموج** وحذّر: `isPriority={index < 2}` يـ mark card 1 كـ priority برضو، لكن mobile (lighthouse formFactor) single-column grid فبيـ render card 0 only above-fold. card 1 below fold بيتـ preloaded على critical path فيـ compete مع الـ actual LCP image — نفس bandwidth-contention pattern اللي Task F القديم (الـ reverted) عمله. **Fix #1** (الـ Codex catch): `isPriority={index < 2}` → `isPriority={index === 0}`. Card 0 only يحصل على priority، card 1+ يـ lazy load (loads quickly after initial paint عشان مش بعيد). بعدها فحص الـ LCP elements الباقية: **/ar/providers/<id>** LCP = `<p class="text-white">` (banner welcome text) بـ render delay **89% (5250ms)**. الـ element في SSR HTML، لكن CustomOrderWelcomeBanner.tsx:121-123 فيه `.animate-banner-shine` decorative overlay (`@keyframes banner-shine` في globals.css:552-563) بـ `animation: ... infinite`. الـ continuous overlay يـ block Lighthouse من finalize LCP لأن الـ painted area على/قرب الـ element بيتغيّر باستمرار. **Fix #2:** `infinite` → `2` (iteration count 2 = 6s total، بعدها overlay يـ park off-screen). الـ shine لسه يظهر للـ user كـ brand flourish لكن LCP يقدر يستقر. **/ar/custom-order** LCP = "إنجزنا" logo نص بـ render delay **87% (3968ms)** — same font-swap issue من Task F (revert) لكن من غير preload الـ blanket. الـ arefRuqaa كان `display: 'swap'` يخلي الـ text يـ paint في fallback ثم swap لـ Aref Ruqaa لما يصل. الـ swap يغيّر glyph metrics → Lighthouse يـ re-evaluate LCP. **Fix #3:** `display: 'swap'` → `display: 'optional'`. الـ optional يـ block ~100ms مع waiting للـ font، لو ما وصلش يـ lock الـ fallback لـ rest of page load (no swap = no metric change = no LCP re-evaluation). الـ trade-off: real users على slow connections يشوفوا fallback من غير Aref Ruqaa على first visit (cached من تاني زيارة). الـ brand text صغير ومقبول للـ trade-off. **التأثير المتوقع:** /ar/providers stable 🥇 (priority على card 0 only without contention)، /ar/providers/<id> LCP −3-4s (banner shine stops blocking)، /ar/custom-order LCP −3-4s (no font swap re-evaluation).
...
- **اكتشاف 2026-05-22 (PR #396 — a11y فشل، TBT noise):** CI run أول كشف TBT على `/ar/welcome` = 735ms (5% over 700ms). محاولة أولى رفعت العتبة 700→800 لكن المراجعة طلبت العلاج من الجذر بدل رفع العتبة. الـ run الثاني (بعد طرح 800) أظهر TBT = 516ms على /ar — أي إن الـ 735 كان variance noise مؤقت داخل الـ band المعلوم. **القرار النهائي:** revert العتبة 800→700، الـ TBT يـ pass باستقرار في الـ run الحالي. لكن الـ run الثاني كشف فشل حقيقي مختلف: `categories:accessibility = 0.86` على /ar و /welcome (تحت 0.9). الـ root cause: (1) WelcomeBoxModal الـ outer dialog div يـ reference `aria-labelledby="welcome-modal-title"` لكن الـ h2 المرتبط يظهر فقط في phases معينة، فالـ dialog يخسر accessible name في loading/error states؛ (2) OffersCarousel + PartnerBannersCarousel pagination dots `<button class="h-1.5">` بدون aria-label وحجم touch target 6px (تحت WCAG AA 44x44). **عُولج في الـ PR الحالي:** WelcomeBoxModal بـ aria-label fallback + 401 → silent close، الـ carousel buttons بـ aria-label dynamic + hit-target padding بالـ before:absolute trick (يحافظ على الـ visual height 6px لكن hit area 44x44).
