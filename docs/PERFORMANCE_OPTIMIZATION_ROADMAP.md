...
> آخر تحديث: 2026-05-12 — Tasks A/B/C/B-bis/C-bis/D/E ✅ merged. **4/8 routes at 🥇 + 1 borderline + 3 🥈**. Task F قيد التنفيذ على `claude/perf-logo-font-preload`: تمكين preload للـ Aref Ruqaa font (45kb، الـ logo فقط) لمعالجة LCP delay الناجم عن فقدان الـ static EngeznaLogo branch للـ font swap (Lighthouse يـ re-evaluate الـ element بعد الـ swap). متوقع تحسين على /custom-order، auth pages، welcome. C-ter REVERTED. render-blocking warn يبقى deferred.
...
| **F**     | Cross-cutting LCP (Task E follow-up): artifact #21 كشف إن `/ar/custom-order` LCP element لسه "إنجزنا" logo بـ render delay 85% — الـ page بيـ redirect لـ /auth/login client-side، فالـ LCP بيتقاس على login page. الـ static EngeznaLogo branch يستخدم `font-family: var(--font-aref-ruqaa)` مع `display: swap` + `preload: false`. الـ fallback→AreRuqaa swap بيغيّر glyph metrics فالـ Lighthouse يـ re-evaluate الـ element كـ LCP جديد بعد الـ swap. **Fix:** `preload: true` على arefRuqaa فقط (45kb single file، مش الـ 8-file 310kb blanket اللي Task C شالها). تأثير متوقع على /custom-order و auth/login و register و forgot-password و كل login pages | `claude/perf-logo-font-preload`       | PR                                          | E merged ✓                               |
...
- **اكتشاف 2026-05-12 (Task F — Aref Ruqaa font swap يلغي LCP timestamp):** بعد Task E merged، artifact #21 أظهر إن `/ar/custom-order` LCP element لسه "إنجزنا" logo بـ render delay 85% (3454ms من 4075ms LCP). فحص الـ filmstrip كشف إن الـ /custom-order page بيـ client-redirect لـ /auth/login للـ unauthenticated users (thumb 2 = loading spinner، thumb 3 = login page بـ "إنجزنا" + sign-in buttons). الـ LCP element selector كان `body... > main#main-content > div.min-h-screen > a.mb-12` — auth/login page structure. الـ login page بيستخدم `<EngeznaLogo size="lg" static showPen={false} />`. الـ static branch مفروض يـ render plain `<span>` بدون animation فمفروض paint مع FCP. لكن الـ static branch CSS فيه `font-family: var(--font-aref-ruqaa)`، والـ font configuration كانت `preload: false` + `display: swap`. الـ swap يخلي الـ text paints في fallback أول، ثم يـ swap للـ Aref Ruqaa لما الـ font يصل. الـ swap بيغيّر glyph metrics بشكل كبير (Aref Ruqaa هو calligraphic font بـ tall x-height vs fallback regular sans serif)، فالـ Lighthouse يـ re-evaluate الـ element كـ LCP candidate جديد بعد الـ swap. الـ LCP timestamp يـ land عند الـ swap moment لا الـ first paint. **عُولج في Task F:** `preload: true` على arefRuqaa فقط في `src/lib/fonts/index.ts`. الـ font ملف واحد 45kb (مش الـ 8-file 310kb blanket اللي Task C شالها)، فالـ critical request path يستوعبه. التأثير المتوقع: LCP improvement على كل pages بـ logo above-the-fold (auth/login، welcome، register، forgot-password، custom-order via redirect). pages بدون logo above-fold (home، providers list، cart) ما هيتأثروش لأن الـ LCP elements عندهم مختلفة (Noto Sans).
