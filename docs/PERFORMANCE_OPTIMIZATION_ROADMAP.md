...
> فرع التنفيذ الحالي للأداء: `claude/perf-css-render-blocking` (Task C-ter — REVERTED. الـ inlineCss و optimizeCss كلاهما wrong لهذا use case — App Router مايدعمش critical-CSS extraction out-of-the-box).
> آخر تحديث: 2026-05-11 — Tasks A/B/C/C-bis ✅ merged. C-ter REVERTED بعد Codex catch ثاني (`inlineCss` يـ inline ALL CSS مش critical، يكسر caching). render-blocking warn يبقى deferred لـ structural fix لاحقاً.
...
| **C-ter** | ❌ DEFERRED — جربنا `optimizeCss` (Pages Router only) و `inlineCss` (App Router، لكن يـ dump الـ full 22KB inline في كل HTML response — يكسر caching ويزيد bytes per request). Next.js لا يدعم critical-CSS extraction للـ App Router out-of-the-box. الـ structural fix يحتاج route-level CSS splitting (substantial refactor) | `claude/perf-css-render-blocking` (revert) | 🔄 PR الحالي = revert                       | —                                        |
