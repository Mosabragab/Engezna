...
> **رسوم التوصيل = حق أصيل للتاجر**
>
> التاجر يدير التوصيل بنفسه، لذا رسوم التوصيل هي إيراد له وليست للمنصة.
> المنصة مجرد وسيط لتحصيلها في حالة الدفع الإلكتروني.
...
cash_flow_classified AS (
    SELECT
        *,
        ...
        -- مستحقات التاجر من طلبات الأونلاين (تشمل رسوم التوصيل!)
        CASE WHEN payment_method = 'online' THEN
            ((net_sales - applied_commission) + delivery_fee) * (1 - refund_percentage)
            -- ⬆️ إضافة delivery_fee لأنها حق التاجر (هو الموصّل)
        ...
        CASE
            ...
            WHEN payment_method = 'cash' THEN
                -- التاجر معه كل شيء (بما في ذلك التوصيل)، المنصة تريد العمولة فقط
    ...
    WHEN payment_method = 'online' THEN
        CASE
            WHEN p.delivery_responsibility = 'merchant' THEN
                -- التاجر يوصّل: أعطه رسوم التوصيل
                ((net_sales - applied_commission) + delivery_fee) * (1 - refund_percentage)
...
4. **المرتجعات**:
   ...
   - تُخصم من وعاء العمولة (المبيعات)
   - رسوم التوصيل تُرد كاملة للعميل
