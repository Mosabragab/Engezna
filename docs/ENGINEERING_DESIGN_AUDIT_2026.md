...
**النسخة:** 3.0 — Deep-Dive Engineering Design Review + Implementation Tracking
**النطاق:** Production Readiness لـ 10,000 مستخدم متزامن
**آخر تحديث للحالة:** 26 فبراير 2026
...
```
خريطة الطريق:  ████████████░░░░░░░░░░░░░  48% مكتمل (12/25 بند)

المرحلة 0 (الأداء الفوري):       ████████████████████░  4.5/5  (90%)
المرحلة 1 (سلامة البيانات):       █████████████████████  5/5    (100%) ✅
المرحلة 2 (Frontend):             █████░░░░░░░░░░░░░░░░  1/4    (25%)
المرحلة 3 (المتانة والمراقبة):     ██░░░░░░░░░░░░░░░░░░░  1/6    (17%)
المرحلة 4 (التحجيم):               █░░░░░░░░░░░░░░░░░░░░  0.5/5  (10%)
```

| الحالة     | العدد | البنود                                           |
| ---------- | ----- | ------------------------------------------------ |
| ✅ مكتمل   | 10    | #2, #3, #4, #5, #6, #7, #8, #9, #10, #19         |
| 🔶 جزئي    | 5     | #1, #13, #14, #18, #21                           |
| ❌ لم يبدأ | 10    | #11, #12, #15, #16, #17, #20, #22, #23, #24, #25 |
...
| -------------------------- | ----------------------------------- | ------------------------------ |
| **التقييم العام**          | **61/100**                          | **~78/100**                    |
| **مشاكل حرجة (CRITICAL)**  | 8                                   | 3 (تم حل 5)                    |
| **مشاكل عالية (HIGH)**     | 11                                  | 7 (تم حل 4)                    |
| **مشاكل متوسطة (MEDIUM)**  | 14                                  | 10 (تم حل 4)                   |
...
| -------------------------------- | -------------- | -------------- | ----------------------------------------- |
| Infrastructure Efficiency & Cost | 45/100         | **72/100**     | ✅ تحسن كبير — Cache + Polling + Realtime |
| Data Integrity & Concurrency     | 50/100         | **88/100**     | ✅ تحسن جذري — Atomic RPC + Guards + RLS  |
| Resiliency & Error Handling      | 62/100         | **65/100**     | 🔶 تحسن طفيف — لا يزال Validation ناقص    |
| Security & Observability         | 72/100         | **78/100**     | 🔶 تحسن — RLS مُصلح لكن Alerting غائب     |
| Frontend Architecture            | 40/100         | **45/100**     | ❌ لا يزال ضعيف — معظم البنود لم تُنفّذ   |
...
| --------------------------- | -------------------------------------- | --------------------------- | --------------------------------------------------------------------------------- |
| **Cache Layer (LRU + TTL)** | `src/lib/cache/cached-queries.ts`      | صفر استخدام في Production   | 🔶 **جزئي** — مُفعّل في `Footer.tsx` فقط. باقي الأماكن لا تزال تستعلم مباشرة      |
...
| **`withErrorHandler`**      | `src/lib/api/error-handler.ts`         | مستخدم في 0 من 45 API route | ❌ **لا يزال غير مُفعّل** — 0 من 45 API route                                     |
| **`withValidation`**        | `src/lib/api/validate.ts`              | مستخدم في 2 من 45 route     | ❌ **لا يزال غير مُفعّل** — 0 API routes تستخدم الـ wrapper                       |
...
> **تحديث:** تم تفعيل 2 من 5 أنظمة (Cache جزئياً، Realtime Manager بالكامل). لكن 3 أنظمة حرجة (`withErrorHandler`، `withValidation`، `strongPasswordSchema`) لا تزال غير مُفعّلة وتُمثّل أعلى ROI متبقي — الكود جاهز ويحتاج فقط ربطه بالـ API routes.
...
| --- | ------------------------------------------------------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | **تفعيل Cache Layer الموجود** لـ governorates, categories, maintenance settings | 🔶 جزئي  | `src/lib/cache/cached-queries.ts` يحتوي `getCachedGovernorates()` و `getCachedBusinessCategories()` و `getCachedDeliveryFees()`. مُفعّل في `Footer.tsx` فقط (ملف واحد). باقي الأماكن التي تستعلم governorates/categories لا تستخدم الـ cache بعد |
...
### المرحلة 2: أداء الـ Frontend (أسبوع 3-4) — 25% مكتمل
...
| 12  | **إضافة `loading.tsx`** لأهم 20 route                             | ❌ لم يبدأ | 4 ملفات `loading.tsx` فقط: `[locale]/loading.tsx`, `[locale]/admin/loading.tsx`, `[locale]/provider/loading.tsx`, `[locale]/auth/callback/loading.tsx`. هذا 3.4% من 117 route — بعيد عن هدف 20 route                                                                                                   |
| 13  | **تقسيم الملفات الكبيرة** (>1500 سطر) بـ `next/dynamic`           | 🔶 جزئي    | الصفحة الرئيسية تستخدم `next/dynamic` لـ `CategoriesSection` (أسطر 12-29). لكن 3 استخدامات فقط لـ `next/dynamic` في كامل المشروع. الملفات الكبيرة لا تزال كما هي: provider/settings (2909 سطر), checkout (2278 سطر), admin/banners (2209 سطر), admin/locations (2017 سطر), provider/finance (1985 سطر) |
...
### المرحلة 3: المتانة والمراقبة (أسبوع 5-6) — 17% مكتمل

| #   | الإجراء                                                 | الحالة     | التفاصيل                                                                                                                                                                                           |
| --- | ------------------------------------------------------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 15  | **تفعيل `withErrorHandler`** في جميع API routes         | ❌ لم يبدأ | `src/lib/api/error-handler.ts` يحتوي الدالة `withErrorHandler()` جاهزة للاستخدام. لكن **0 من 45** API route يستخدمها فعلياً. جميع الـ routes تستخدم try-catch يدوي                                 |
| 16  | **تفعيل `withValidation`** في جميع API routes           | ❌ لم يبدأ | `src/lib/api/validate.ts` يُصدّر `withValidation()`. **0 API routes** تستخدم هذا الـ wrapper. endpoint التسجيل (`auth/register/route.ts`) يعمل validation يدوي بدلاً من استخدام الـ wrapper        |
...
### المرحلة 4: التحجيم (أسبوع 7-8) — 10% مكتمل
...
| 22  | **تفعيل `strongPasswordSchema`** في auth                  | ❌ لم يبدأ | `src/lib/validations/common.ts` يُعرّف `strongPasswordSchema` لكنها غير مستوردة أو مُستخدمة في أي صفحة auth. صفحات التسجيل وإعادة تعيين كلمة المرور لا تتحقق مقابل هذا الـ schema                                                      |
| 23  | **إصلاح error boundaries locale** — استخراج locale من URL | ❌ لم يبدأ | Error boundaries تحتوي روابط hardcoded لـ `/ar`: `error.tsx` سطر 95 `href="/ar"`، `provider/error.tsx` تحتوي `href="/ar/provider"`، `admin/error.tsx` نفس المشكلة. يجب استخراج locale ديناميكياً من URL                                |
| 24  | **إضافة Edge config data scrubbing**                      | ❌ لم يبدأ | `sentry.edge.config.ts` (أسطر 33-47) يحتوي `beforeSend()` يُرشّح الأخطاء الشائعة، لكن **لا يوجد data scrubbing** لـ auth headers أو البيانات الحساسة كما في server config. الـ headers قد تتسرب لـ Sentry من middleware                |
| 25  | **تفعيل Upstash لـ promo validation** بدل in-memory       | ❌ لم يبدأ | `src/app/api/promo/validate/route.ts` (أسطر 34-50) يستخدم rate limiting in-memory (`rateLimitMap`). هذا لا يعمل عبر serverless instances. لم يتم الترحيل لـ Upstash Redis كما في Chat API                                              |
...
| -------- | ------------------------------------------------------- | ------------------------------------------------------------------------ | ------- |
| 🔴 1     | **#15+16: تفعيل `withErrorHandler` و `withValidation`** | الكود **جاهز** ويحتاج فقط ربطه بـ 45 API route. أعلى ROI متبقي           | 20 ساعة |
| 🔴 2     | **#1: توسيع استخدام Cache Layer**                       | مُستخدم في مكان واحد فقط (`Footer.tsx`). يحتاج تفعيل في كل الأماكن       | 4 ساعات |
...
| 🟠 5     | **#12: إضافة `loading.tsx`** لأهم 20 route              | 4 فقط من 117 route — شاشة بيضاء لمعظم المستخدمين                         | 8 ساعات |
| 🟡 6     | **#11: تحويل Home Page لـ Server Component**            | لا يزال `'use client'` — يؤثر على SEO و LCP                              | 12 ساعة |
| 🟡 7     | **#22-25: بنود التحجيم الصغيرة**                        | `strongPasswordSchema`, error boundaries locale, edge scrubbing, Upstash | 5 ساعات |

### القرارات الهندسية التي لا تزال مطلوبة

1. **`withErrorHandler` و `withValidation` لا يزالان غير مُفعّلين** — هذا يبقى أخطر قرار هندسي في المشروع. الأنظمة مبنية بشكل ممتاز لكن الفجوة بين "مبني" و "مُفعّل" تعطي إحساساً زائفاً بالأمان
2. **الـ Frontend لا يزال ضعيفاً** — 95.7% client-rendered مع 4 `loading.tsx` فقط من 117 route
...
_نسبة الإنجاز: 48% (10 مكتمل، 5 جزئي، 10 لم يبدأ)_
