 ...
 */
function createEmbeddingText(item: MenuItem, categoryName?: string): string {
  const parts: string[] = [];
...
async function main() {
  ...
  const { data: items, error: itemsError } = await supabase
    .from('menu_items')
    .select(
      'id, name_ar, name_en, description_ar, description_en, is_spicy, is_vegetarian, provider_category_id'
    )
