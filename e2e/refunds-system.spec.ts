  ...
  test('should have refund form elements visible', async ({ page }) => {
    ...
    if (page.url().includes('/orders') && !page.url().includes('/login')) {
      ...
      const orderCard = page
        .locator('[class*="order"], [class*="Order"], a[href*="/orders/"]')
        .first();
  ...
  test('should have refund action buttons visible', async ({ page }) => {
    ...
    if (page.url().includes('/refunds') && !page.url().includes('/login')) {
      ...
      const approveBtn = page.locator(
        'button:has-text("موافقة"), button:has-text("قبول"), button:has-text("Approve")'
      );
      ...
      const hasApproveBtn = await approveBtn
        ...
        .isVisible()
        .catch(() => false);
      const hasRejectBtn = await rejectBtn
        .first()
        .isVisible()
        .catch(() => false);
  ...
  test('should show refund details', async ({ page }) => {
    ...
    if (page.url().includes('/refunds') && !page.url().includes('/login')) {
      ...
      if (await refundItem.isVisible().catch(() => false)) {
        ...
        const hasRefundInfo =
          itemText?.includes('#') || // Order number
...
test.describe('Refund Impact on Finance', () => {
  ...
  test('should have refund filter in transaction history', async ({ page }) => {
    ...
    if (page.url().includes('/finance') && !page.url().includes('/login')) {
      ...
      const refundFilter = page.locator(
        'button:has-text("مرتجع"), button:has-text("Refund"), button:has-text("المرتجعات")'
      );

      const hasRefundFilter = await refundFilter
        .first()
        .isVisible()
        .catch(() => false);
  ...
  test('commission card should account for refunds', async ({ page }) => {
    ...
    if (page.url().includes('/finance') && !page.url().includes('/login')) {
      ...
      const commissionCard = page.locator(
        '[class*="card"]:has-text("عمولة"), [class*="card"]:has-text("commission")'
      );

      if (
        await commissionCard
          .first()
          .isVisible()
          .catch(() => false)
      ) {
        ...
        const hasAmount =
          cardText?.includes('ج.م') || cardText?.includes('EGP') || /\d+/.test(cardText || '');
    ...
    if (url.includes('/provider')) {
      ...
      if (hasSidebar) {
        ...
        const badges = sidebar.locator(
          '[class*="badge"], [class*="rounded-full"], span[class*="bg-"]'
        );
