  ...
  test('should have date filter options', async ({ page }) => {
    ...
    if (page.url().includes('/finance') && !page.url().includes('/login')) {
      ...
      const dateFilters = page.locator(
        'button:has-text("اليوم"), button:has-text("Today"), button:has-text("الأسبوع"), button:has-text("Week"), button:has-text("الشهر"), button:has-text("Month")'
      );
  ...
  test('should show collection progress for COD', async ({ page }) => {
    ...
    if (page.url().includes('/finance') && !page.url().includes('/login')) {
      ...
      const progressBars = page.locator(
        '[class*="progress"], [class*="h-2"][class*="rounded-full"]'
      );
...
test.describe('Transaction History', () => {
  ...
  test('should have transaction type filters', async ({ page }) => {
    ...
    if (page.url().includes('/finance') && !page.url().includes('/login')) {
      ...
      const filters = page.locator(
        'button:has-text("الكل"), button:has-text("All"), button:has-text("طلبات"), button:has-text("Orders"), button:has-text("مرتجعات"), button:has-text("Refunds")'
      );
  ...
  test('should display transaction items with correct icons', async ({ page }) => {
    ...
    if (page.url().includes('/finance') && !page.url().includes('/login')) {
      ...
      const pageContent = await page.textContent('body');
      const hasEmptyState =
        pageContent?.includes('لا توجد') || pageContent?.includes('no transactions');
  ...
  test('should show refund transactions in red', async ({ page }) => {
    ...
    if (page.url().includes('/finance') && !page.url().includes('/login')) {
      ...
      const refundFilter = page
        .locator('button:has-text("مرتجعات"), button:has-text("Refunds")')
        .first();
...
test.describe('Provider Reports Page', () => {
  ...
  test('should show correct revenue calculations', async ({ page }) => {
    ...
    if (page.url().includes('/reports') && !page.url().includes('/login')) {
      ...
      const revenueCards = page.locator(
        '[class*="card"]:has-text("ج.م"), [class*="card"]:has-text("EGP")'
      );
...
test.describe('Finance Responsive Design', () => {
  ...
  test('should stack cards on mobile', async ({ page }) => {
    ...
    if (page.url().includes('/finance') && !page.url().includes('/login')) {
      ...
      const firstCardVisible = await cards
        .first()
        .isVisible()
        .catch(() => false);
