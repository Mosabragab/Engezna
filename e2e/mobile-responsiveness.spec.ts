...
async function checkTouchTargets(
  ...
  valid: number;
  invalid: { selector: string; size: { width: number; height: number } }[];
}> {
    ...
    for (let i = 0; i < rects.length; i++) {
      for (let j = i + 1; j < rects.length; j++) {
        ...
        if (overlapX > 10 && overlapY > 10) {
          overlaps.push(`Overlap detected: ${rects[i].el.tagName} and ${rects[j].el.tagName}`);
        }
      }
    }
...
test.describe('Mobile Responsiveness - Android Generic', () => {
  ...
  test.describe('Customer Pages', () => {
    ...
    test('Cart page - Quantity controls accessible (48px)', async ({ page }) => {
      ...
      const qtyButtons = page
        .locator(LOCATORS.increaseButton)
