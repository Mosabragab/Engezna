...
test.describe('Shopping Cart', () => {
  ...
  test('should have quantity controls', async ({ page }) => {
    ...
    const hasPlusBtn = await plusBtn
      .first()
      .isVisible()
      .catch(() => false);
    const hasMinusBtn = await minusBtn
      .first()
      .isVisible()
      .catch(() => false);
  ...
  test('should have remove item button', async ({ page }) => {
    ...
    const removeBtn = page.locator(
      'button:has-text("حذف"), button:has-text("Remove"), button:has(svg[class*="trash"])'
    );
  ...
  test('should have checkout button', async ({ page }) => {
    ...
    const checkoutBtn = page.locator(
      'button:has-text("الدفع"), button:has-text("Checkout"), a[href*="/checkout"]'
    );

    const hasCheckoutBtn = await checkoutBtn
      .first()
      .isVisible()
      .catch(() => false);
  ...
  test('should have place order button when on checkout', async ({ page }) => {
    ...
    if (url.includes('/checkout')) {
      ...
      const orderBtn = page.locator(
        'button:has-text("تأكيد"), button:has-text("Confirm"), button:has-text("طلب"), button:has-text("Order")'
      );

      const hasOrderBtn = await orderBtn
        .first()
        .isVisible()
        .catch(() => false);
  ...
  test('should navigate to order detail page', async ({ page }) => {
    ...
    if (url.includes('/orders')) {
      ...
      const orderLink = page
        .locator('a[href*="/orders/"], [class*="order"] a, [class*="Order"] a')
        .first();
      ...
      if (await orderLink.isVisible().catch(() => false)) {
        ...
        const isOrderDetail = /\/orders\/[a-zA-Z0-9-]+/i.test(newUrl) || newUrl.includes('/order/');
  ...
  test('should have add address button', async ({ page }) => {
    ...
    if (page.url().includes('/addresses') && !page.url().includes('/login')) {
      ...
      const addBtn = page.locator(
        'button:has-text("إضافة"), button:has-text("Add"), a[href*="add"]'
      );
