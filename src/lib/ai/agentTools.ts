...
export const AGENT_TOOLS: ToolDefinition[] = [
  ...
  },
  {
    name: 'search_menu',
    description:
      'البحث في المنيو - يدور في اسم المنتج والوصف واسم القسم (مثال: "حلويات" هيجيب كل منتجات قسم الحلويات)',
    parameters: {
      type: 'object',
      properties: {
        provider_id: {
  ...
  },
  {
    ...
    description:
      'إزالة منتج من السلة - استخدمها لما العميل يقول "شيل" أو "الغي" أو "امسح" منتج معين',
    parameters: {
      type: 'object',
      properties: {
        item_name: {
  ...
  },
  {
    ...
    description:
      'عرض محتويات السلة الحالية - استخدمها لما العميل يقول "ايه في السلة" أو "كام الحساب"',
    parameters: {
      type: 'object',
      properties: {},
      required: [],
  ...
  },
  {
    ...
    description:
      'الحصول على معلومات التوصيل (رسوم التوصيل، الحد الأدنى، الوقت المتوقع) - لو مفيش provider_id هيستخدم تاجر السلة',
    parameters: {
      type: 'object',
      properties: {
        provider_id: {
  ...
  },
  {
    ...
    description:
      'البحث عن تاجر بالاسم والحصول على معرفه - استخدمها لما العميل يختار تاجر معين بالاسم (مثال: "الصفا", "سلطان بيتزا")',
    parameters: {
      type: 'object',
      properties: {
        provider_name: {
  ...
  },
  {
    ...
    description:
      'الحصول على أقسام البيزنس المتاحة (مطاعم، سوبر ماركت، خضار، بن) - استخدمها لما العميل يسأل عن الأقسام أو يختار قسم',
    parameters: {
      type: 'object',
      properties: {},
      required: [],
  ...
  // ─────────────────────────────────────────────────────────────────────────
  {
    ...
    description:
      'الحصول على كل العروض المتاحة - يرجع 3 أنواع: (1) أكواد الخصم promo_codes زي WELCOME30, SAVE20 (2) عروض التاجر promotions زي خصم نهاية الأسبوع (3) المنتجات المخفضة discounted_products. لو مفيش provider_id هيجيب أكواد الخصم العامة بس',
    parameters: {
      type: 'object',
      properties: {
        provider_id: {
  ...
  try {
    switch (toolName) {
      ...
      case 'get_provider_categories': {
        ...
        // ═══════════════════════════════════════════════════════════════════
        const effectiveProviderId = getValidUUID(
          param_provider_id,
          context.providerId,
          context.cartProviderId
        );
      ...
      case 'get_menu_items': {
        ...
        if (search_query) {
          ...
          if (searchResult.error) {
            ...
            const fallbackResult = await supabase
              .from('menu_items')
              .select(
                'id, name_ar, name_en, description_ar, price, original_price, image_url, is_available, has_stock, has_variants, pricing_type, category_id'
              )
              .eq('provider_id', effectiveProviderId)
              .eq('is_available', true)
              .or(
                `name_ar.ilike.%${search_query}%,name_ar.ilike.%${normalizeArabic(search_query)}%`
              )
              .order('display_order')
              .limit(limit);
        ...
        if (itemsWithVariants.length > 0) {
          ...
          const itemsMap = new Map(
            data?.map((item) => [item.id, { ...item, variants: [] as typeof variants }])
          );
      ...
      case 'search_menu': {
        ...
        const fetchVariantsForItems = async (
          items: Array<{ id: string; has_variants: boolean | null }>
        ) => {
          ...
          const deduplicatedVariants = variants?.filter((variant, index, arr) => {
            ...
            return (
              arr.findIndex(
                (v) => v.product_id === variant.product_id && v.name_ar === variant.name_ar
              ) === index
            );
          });
          ...
          // Attach variants to items
          const itemsMap = new Map(
            items.map((item) => [item.id, { ...item, variants: [] as typeof deduplicatedVariants }])
          );
        ...
        const findMatchingCategoryIds = async (
          searchQuery: string,
          providerIds?: string[]
        ): Promise<string[]> => {
        ...
        try {
          ...
          if (queryEmbedding) {
            ...
            const result = await supabase.rpc('simple_search_menu', {
              p_query: query,
              p_provider_id: effectiveProviderId || null,
              p_city_id: effectiveCityId || null,
              p_limit: 15,
          ...
          if (!hybridError && hybridResults && hybridResults.length > 0) {
            ...
            const formattedResults = hybridResults.map(
              (item: {
                id: string;
                name_ar: string;
                name_en: string;
            ...
            if (!effectiveProviderId) {
              ...
              formattedResults.forEach(
                (item: { provider_id: string; providers: { name_ar: string } }) => {
              ...
              if (uniqueProviders.length === 1) {
                ...
                const providerItems = formattedResults
                  .filter((item: { provider_id: string }) => item.provider_id === providerId)
                  .slice(0, 10);
              ...
              return {
                ...
                discovered_provider_id:
                  uniqueProviders.length === 1 ? uniqueProviders[0].id : undefined,
                discovered_provider_name:
                  uniqueProviders.length === 1 ? uniqueProviders[0].name_ar : undefined,
              };
            }
            ...
            const fromDifferentProvider =
              ...
              formattedResults.every(
                (item: { provider_id: string }) => item.provider_id !== effectiveProviderId
              );
        ...
        if (effectiveProviderId) {
          ...
          if (matchingCategoryIds.length > 0) {
            ...
            searchQuery = searchQuery.or(
              `name_ar.ilike.*${query}*,description_ar.ilike.*${query}*,category_id.in.(${matchingCategoryIds.join(',')})`
            );
          } else {
            searchQuery = searchQuery.or(
          ...
          if (otherProviders?.length) {
            ...
            const globalCategoryIds = await findMatchingCategoryIds(
              query,
              otherProviders.map((p) => p.id)
            );
            ...
            if (globalCategoryIds.length > 0) {
              globalSearchQuery = globalSearchQuery.or(
                `name_ar.ilike.*${query}*,description_ar.ilike.*${query}*,category_id.in.(${globalCategoryIds.join(',')})`
              );
            } else {
              globalSearchQuery = globalSearchQuery.or(
                `name_ar.ilike.*${query}*,description_ar.ilike.*${query}*`
          ...
          let providersQuery = supabase
            .from('providers')
            .select(
              'id, name_ar, logo_url, rating, total_reviews, delivery_fee, estimated_delivery_time_min, category, status'
            )
            .in('status', ['open', 'closed', 'temporarily_paused']);
          ...
          const allCategoryIds = await findMatchingCategoryIds(
            query,
          ...
          const previousProviderIds =
            context.customerMemory?.lastOrders?.map((o) => o.providerId) || [];
      ...
      case 'check_item_availability': {
        ...
        return {
          ...
          data: {
            ...
            item: data,
            message: isAvailable ? 'المنتج متاح' : data?.stock_notes || 'المنتج غير متاح حالياً',
          },
        };
      }
      ...
      case 'add_to_cart': {
        ...
        if (!provider_id || provider_id === 'undefined' || provider_id === 'null') {
          console.error(
            '[add_to_cart] Missing provider_id (no fallback available):',
            param_provider_id
          );
          return {
            success: false,
        ...
        const { data: item, error: itemError } = await supabase
          .from('menu_items')
          .select(
            'id, name_ar, is_available, has_stock, stock_notes, price, provider_id, has_variants'
          )
          .eq('id', item_id)
          .single();
        ...
        if (item.has_variants && !variant_id) {
          ...
          const variantsList =
            variants?.map((v) => `• ${v.name_ar}: ${v.price} ج.م (id: ${v.id})`).join('\n') || '';
      ...
      case 'check_provider_open': {
        ...
        if (data?.status === 'closed' || data?.status === 'temporarily_paused') {
          ...
          message = isOpen ? `مفتوح حتى ${closeTime}` : `مغلق - يفتح الساعة ${openTime}`;
        }
      ...
      case 'get_delivery_info': {
        ...
        const { data, error } = await supabase
          .from('providers')
          .select(
            'id, name_ar, delivery_fee, min_order_amount, estimated_delivery_time_min, delivery_radius_km'
          )
      ...
      case 'lookup_provider': {
        ...
        const matches = providers.filter((p) => {
          ...
          return (
            normalizedProviderName.includes(normalizedSearchName) ||
        ...
        if (matches.length === 0) {
          ...
          const partialMatches = providers.filter((p) => {
            ...
            return words.some((word) =>
              normalizeArabic(p.name_ar?.toLowerCase() || '').includes(
                normalizeArabic(word.toLowerCase())
              )
            );
          });
      ...
      case 'get_provider_promotions': {
        ...
        const { data: promoCodes } = await supabase
          .from('promo_codes')
          .select(
            'id, code, description_ar, discount_type, discount_value, min_order_amount, max_discount_amount, first_order_only, applicable_providers'
          )
          .eq('is_active', true)
          .lte('valid_from', now)
          .gte('valid_until', now);
        ...
        let promotionsQuery = supabase
          .from('promotions')
          .select(
            'id, name_ar, name_en, type, discount_value, min_order_amount, max_discount, start_date, end_date, provider_id, providers(name_ar)'
          )
          .eq('is_active', true)
        ...
        promotions = (promotionsData || [])
          ...
          .map((promo) => {
            ...
            const providerName =
              providers?.name_ar || (Array.isArray(providers) ? providers[0]?.name_ar : undefined);
        ...
        let discountQuery = supabase
          .from('menu_items')
          .select(
            'id, name_ar, price, original_price, image_url, has_variants, provider_id, providers(name_ar)'
          )
          .eq('is_available', true)
        ...
        productsWithDiscount =
          discountedProducts
            ...
            .map((p) => {
              ...
              const providerName =
                providers?.name_ar ||
                (Array.isArray(providers) ? providers[0]?.name_ar : undefined);
...
export function analyzeConversationForInsights(
  ...
  const changeCount = userMessages.filter(
    (m) => m.includes('غير') || m.includes('بدل') || m.includes('مش عايز')
  ).length;

  const questionCount = userMessages.filter(
    (m) => m.includes('؟') || m.includes('ايه') || m.includes('كام')
  ).length;
