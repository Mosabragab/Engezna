...
export function deduplicateCartActions(
  ...
  const filteredActions = actions.filter(action => {
    ...
    if (existingItem) {
      skipped.push(action)
      reasons.push(`${action.menu_item_name_ar} موجود في السلة أصلاً (${existingItem.quantity} قطعة)`)
      return false
    }
...
export function preProcessMessage(message: string): PreProcessedMessage {
  ...
  const isConfirmation = /^(أيوه|ايوه|تمام|ماشي|ضيفهم|اضيفهم|أكيد|اكيد|صح|اه|آه|نعم)/.test(message.trim())
...
export function postProcessCartActions(
  ...
  const { actions: dedupedActions, skipped, reason } = deduplicateCartActions(mergedActions, existingCart)
