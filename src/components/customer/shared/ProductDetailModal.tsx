...
export function ProductDetailModal({
  ...
  const [selectedVariant, setSelectedVariant] = useState<ProductVariant | null>(
    hasVariants ? (product.variants?.find(v => v.is_default) || product.variants?.[0] || null) : null
  )
        ...
        <div className="p-4 space-y-4">
          ...
          {!hasVariants && (
            <div className="flex items-center gap-2">
              <span className="text-2xl font-bold text-primary">{product.price} {currency}</span>
              {hasDiscount && (
                <span className="text-slate-400 line-through">{product.original_price} {currency}</span>
              )}
            </div>
          )}
