...
import { Search, X, User, Store, ShoppingBag, ChevronDown, Building, Map, Check } from 'lucide-react'
...
export function SearchableSelect({
  ...
  const getDefaultPlaceholder = () => {
    switch (type) {
      case 'customer': return locale === 'ar' ? 'اختر العميل' : 'Select customer'
      case 'provider': return locale === 'ar' ? 'اختر المتجر' : 'Select provider'
      case 'order': return locale === 'ar' ? 'اختر الطلب' : 'Select order'
    }
  }
            ...
            {loading ? (
              ...
              filteredItems.map((item) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`font-medium truncate ${item.id === value ? 'text-red-600' : 'text-slate-900'}`}>
                      {item.label}
                    </p>
                    {item.sublabel && (
