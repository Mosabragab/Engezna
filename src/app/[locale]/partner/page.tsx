  ...
  return (
    <div className="min-h-screen bg-white">
            ...
            <div className="max-w-4xl mx-auto text-center">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
                <Link href={`/${locale}/partner/register`}>
                  <Button
                    size="lg"
                    className="bg-[#009DE0] hover:bg-[#0086c3] text-white px-8 py-6 text-lg rounded-xl shadow-lg shadow-[#009DE0]/25 transition-all hover:scale-105"
                  >
                ...
                <Link href={`/${locale}/provider/login`}>
                  <Button
                    variant="outline"
                    size="lg"
                    className="px-8 py-6 text-lg rounded-xl border-[#009DE0] text-[#009DE0] hover:bg-[#009DE0] hover:text-white transition-all duration-200"
                  >
                    {t('login')}
                  </Button>
                </Link>
              ...
              <div className="grid grid-cols-3 gap-6 md:gap-12 max-w-xl mx-auto">
                {stats.map((stat, index) => (
                  ...
                  >
                    ...
                    <div className="text-3xl md:text-4xl font-bold text-[#0F172A]">
                      {stat.value}
                    </div>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
            {benefits.map((benefit, index) => (
              ...
              >
                <div
                  className={`w-14 h-14 ${benefit.bgColor} rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300`}
                >
              ...
              {steps.map((step, index) => (
                <div key={index} className="relative text-center">
                  ...
                  {index < steps.length - 1 && (
                    <div
                      className={`hidden md:block absolute top-10 ${isRTL ? 'right-0 -translate-x-1/2' : 'left-full -translate-x-1/2'} w-full h-0.5 bg-gradient-to-r from-[#009DE0]/30 to-[#009DE0]/10`}
                    />
                  )}
      ...
      <section id="faq" className="py-16 md:py-24 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-[#0F172A] mb-4">{t('faqTitle')}</h2>
          </div>
            ...
            {faqs.map((faq, index) => (
              ...
              >
                ...
                >
                  ...
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${openFaq === index ? 'bg-primary/10 rotate-180' : 'bg-slate-100'}`}
                  >
                    <ChevronDown
                      className={`w-4 h-4 transition-colors ${openFaq === index ? 'text-primary' : 'text-slate-500'}`}
                    />
                  </div>
                </button>
                <div
      ...
      <section className="py-16 md:py-24 bg-gradient-to-br from-[#009DE0] via-[#0086c3] to-[#006d9e] relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          <div className="max-w-2xl mx-auto">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">{t('readyToJoin')}</h2>
            <p className="text-xl text-white/90 mb-8">{t('readyToJoinSubtitle')}</p>
            <Link href={`/${locale}/partner/register`}>
              <Button
                size="lg"
                className="bg-white text-[#009DE0] hover:bg-slate-50 hover:scale-105 px-8 py-6 text-lg rounded-2xl shadow-elegant-lg transition-all duration-300"
              >
                <Store className="w-5 h-5 me-2" strokeWidth={1.8} />
                {t('registerFree')}
              </Button>
