...
const STATUS_CONFIG: Record<
  string,
  { icon: LucideIcon; color: string; bgColor: string; label_ar: string; label_en: string }
> = {
  pending: {
    icon: Clock,
    color: 'text-[hsl(48_97%_40%)]',
    bgColor: 'bg-[hsl(48_100%_95%)]',
    label_ar: 'طلب جديد',
    label_en: 'New Order',
  },
  accepted: {
  ...
  ready: {
    icon: Package,
    color: 'text-primary',
    bgColor: 'bg-[hsl(198_100%_44%/0.1)]',
    label_ar: 'جاهز',
    label_en: 'Ready',
  },
  out_for_delivery: {
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      if (existingOrder.provider_id !== providerId) {
        alert(
          locale === 'ar'
            ? 'خطأ: ليس لديك صلاحية تحديث هذا الطلب'
      ...
      if (updateError) {
        alert(
          locale === 'ar'
            ? 'خطأ في تحديث حالة الدفع: ' + updateError.message
              ...
              return (
                ...
                >
                  <CardContent className="p-0">
                    {/* Order Header */}
                    <div
                      className={`p-4 border-b border-slate-100 ${order.status === 'pending' ? 'bg-gradient-to-r from-amber-50/80 to-yellow-50/50' : ''}`}
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="text-end">
                          <p className="text-sm text-slate-500">{formatTime(order.created_at)}</p>
                      ...
                      <div className="space-y-2">
                        ...
                        {order.items.length > 3 && (
                          <p className="text-xs text-slate-500">
                            +{order.items.length - 3}{' '}
                            {locale === 'ar' ? 'منتجات أخرى' : 'more items'}
                          </p>
                        )}
                      </div>
                    ...
                    <div className="p-4 flex items-center justify-between">
                      <div>
                        <p className="text-2xl font-bold text-primary">
                          ...
                          <span className="text-sm">{locale === 'ar' ? 'ج.م' : 'EGP'}</span>
                        </p>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-xl">
            ...
            <div className="flex gap-3">
              ...
              <Button className="flex-1 bg-deal hover:bg-deal/90" onClick={handleConfirmPayment}>
