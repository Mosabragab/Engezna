...
function TeamMemberCard({
  ...
  const permissions = [
    {
      key: 'orders',
      enabled: member.can_manage_orders,
      label: locale === 'ar' ? 'الطلبات' : 'Orders',
    },
    {
      key: 'menu',
      enabled: member.can_manage_menu,
      label: locale === 'ar' ? 'المنتجات' : 'Products',
    },
    {
      key: 'customers',
      enabled: member.can_manage_customers,
      label: locale === 'ar' ? 'العملاء' : 'Customers',
    },
    {
      key: 'analytics',
      enabled: member.can_view_analytics,
      label: locale === 'ar' ? 'التحليلات' : 'Analytics',
    },
    {
      key: 'offers',
      enabled: member.can_manage_offers,
    ...
    <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200/60 p-5 shadow-elegant hover:shadow-elegant-lg transition-all duration-300">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          `}
          >
            {isOwner ? <Crown className="w-7 h-7" /> : <UserCheck className="w-7 h-7" />}
          </div>

          {/* Name & Email */}
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-slate-900">{member.profile.full_name}</h3>
              {isOwner && (
        ...
        <div className="flex flex-wrap gap-2">
          {isOwner ? (
            ...
            permissions.map((perm) => (
              <PermissionBadge key={perm.key} enabled={perm.enabled} label={perm.label} />
            ))
          )}
        </div>
      </div>
...
export default function TeamManagementPage() {
  ...
  const handleResendInvitation = async (invitation: PendingInvitation) => {
    ...
    try {
      ...
      await supabase.from('provider_invitations').delete().eq('id', invitation.id);
  ...
  if (!permissionsLoading && !isOwner) {
    return (
      <ProviderLayout pageTitle={{ ar: 'إدارة الفريق', en: 'Team Management' }}>
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200/60 p-8 text-center shadow-elegant">
    ...
    >
      ...
      <div className="space-y-4 mb-8">
        ...
        {staffMembers.length > 0 ? (
          ...
          <EmptyState locale={locale} onAddStaff={() => setShowAddModal(true)} />
        )}
      </div>
