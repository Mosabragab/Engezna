...
type ApprovalType = 'refund' | 'customer_ban' | 'provider_suspend' | 'commission_change' | 'settlement_adjust' | 'promo_create' | 'other'
...
// Status config
const statusConfig: Record<ApprovalStatus, { label: { ar: string; en: string }; color: string; icon: React.ElementType }> = {
  pending: { label: { ar: 'في الانتظار', en: 'Pending' }, color: 'bg-yellow-100 text-yellow-700', icon: Clock },
  approved: { label: { ar: 'موافق عليه', en: 'Approved' }, color: 'bg-green-100 text-green-700', icon: CheckCircle2 },
  approved_with_changes: { label: { ar: 'موافق مع تعديل', en: 'Approved w/ Changes' }, color: 'bg-blue-100 text-blue-700', icon: CheckCircle2 },
  rejected: { label: { ar: 'مرفوض', en: 'Rejected' }, color: 'bg-red-100 text-red-700', icon: XCircle },
  cancelled: { label: { ar: 'ملغى', en: 'Cancelled' }, color: 'bg-slate-100 text-slate-700', icon: XCircle },
}

const priorityConfig: Record<ApprovalPriority, { label: { ar: string; en: string }; color: string }> = {
...
const typeConfig: Record<ApprovalType, { label: { ar: string; en: string }; icon: React.ElementType; color: string }> = {
  refund: { label: { ar: 'استرداد مالي', en: 'Refund' }, icon: DollarSign, color: 'text-emerald-600' },
  customer_ban: { label: { ar: 'حظر عميل', en: 'Customer Ban' }, icon: Ban, color: 'text-red-600' },
  provider_suspend: { label: { ar: 'تعليق متجر', en: 'Provider Suspend' }, icon: Store, color: 'text-orange-600' },
  commission_change: { label: { ar: 'تعديل عمولة', en: 'Commission Change' }, icon: Settings, color: 'text-blue-600' },
  settlement_adjust: { label: { ar: 'تعديل تسوية', en: 'Settlement Adjust' }, icon: FileText, color: 'text-purple-600' },
  promo_create: { label: { ar: 'إنشاء عرض', en: 'Create Promo' }, icon: ShoppingBag, color: 'text-pink-600' },
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
            ...
            <div className="bg-yellow-50 rounded-xl p-4 border border-yellow-200">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-sm text-yellow-700">{locale === 'ar' ? 'في الانتظار' : 'Pending'}</span>
              </div>
            ...
            <div className="bg-green-50 rounded-xl p-4 border border-green-200">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-sm text-green-700">{locale === 'ar' ? 'موافق عليها' : 'Approved'}</span>
              </div>
              <p className="text-2xl font-bold text-green-700">{formatNumber(stats.approved, locale)}</p>
            </div>
            <div className="bg-red-50 rounded-xl p-4 border border-red-200">
              <div className="flex items-center gap-3 mb-2">
                <XCircle className="w-5 h-5 text-red-600" />
                <span className="text-sm text-red-700">{locale === 'ar' ? 'مرفوضة' : 'Rejected'}</span>
              </div>
              <p className="text-2xl font-bold text-red-700">{formatNumber(stats.rejected, locale)}</p>
            </div>
            <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
              <div className="flex items-center gap-3 mb-2">
                <UserIcon className="w-5 h-5 text-blue-600" />
                <span className="text-sm text-blue-700">{locale === 'ar' ? 'طلباتي' : 'My Requests'}</span>
              </div>
              <p className="text-2xl font-bold text-blue-700">{formatNumber(stats.myRequests, locale)}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm mb-6">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              >
                ...
                {stats.pending > 0 && (
                  <span className="ms-2 bg-white/20 px-2 py-0.5 rounded-full text-xs">{stats.pending}</span>
                )}
              </Button>
              <Button
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-sm font-mono text-slate-500">#{approval.request_number}</span>
                            <span className={`px-2 py-0.5 rounded text-xs font-medium ${priorityConfig[approval.priority]?.color}`}>
                              {priorityConfig[approval.priority]?.label[locale === 'ar' ? 'ar' : 'en']}
                            </span>
                            <span className="text-xs text-slate-400">{getTimeSince(approval.created_at)}</span>
                          </div>
                          ...
                          <p className="text-sm text-slate-500 mt-1">
                            ...
                            {approval.amount && (
                              <span className="ms-2 font-medium text-slate-700">
                                | {formatCurrency(approval.amount, locale)} {locale === 'ar' ? 'ج.م' : 'EGP'}
                              </span>
                            )}
                          </p>
                        </div>
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${statusConfig[approval.status]?.color}`}>
                    ...
                    <div className="p-4 bg-slate-50">
                      ...
                      {approval.requester && (
                        <div className="flex items-center gap-2 text-sm text-slate-600 mb-3">
                          ...
                          <span className="font-medium text-slate-900">{approval.requester.full_name}</span>
                        </div>
                      )}
                      ...
                      {approval.decision_notes && (
                        <div className={`rounded-lg p-3 border mb-3 ${approval.status === 'approved' || approval.status === 'approved_with_changes' ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                          <p className={`text-sm font-medium mb-1 ${approval.status === 'approved' || approval.status === 'approved_with_changes' ? 'text-green-700' : 'text-red-700'}`}>
                            {locale === 'ar' ? 'قرار المدير:' : 'Manager Decision:'}
                          </p>
                          <p className={`text-sm ${approval.status === 'approved' || approval.status === 'approved_with_changes' ? 'text-green-700' : 'text-red-700'}`}>
                            {approval.decision_notes}
                          </p>
                          {approval.decider && (
                            <p className="text-xs mt-2 text-slate-500">
                              - {approval.decider.full_name}, {formatDate(approval.decided_at!, locale)}
                            </p>
                          )}
                        </div>
                      )}
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
            ...
            <div className="mb-4">
              ...
              <textarea
                ...
                placeholder={locale === 'ar' ? 'أضف ملاحظات على قرارك...' : 'Add notes to your decision...'}
                rows={4}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFormData({ ...formData, priority: e.target.value as ApprovalPriority })}
                    className="w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-red-500"
                  >
