...
export default function AdminRegionalAnalyticsPage() {
  ...
  async function loadRegionData(supabase: ReturnType<typeof createClient>) {
    ...
    const prevStartDate = getDateRange(periodFilter === 'week' ? 'month' : periodFilter === 'month' ? 'quarter' : 'year')

    // Get orders with address information
    const { data: ordersData } = await supabase
      .from('orders')
      .select('id, total, platform_commission, created_at, status, customer_id, provider_id, delivery_address')
    ...
    if (viewLevel === 'governorates') {
      ...
      stats = filteredDistricts.map(district => {
    ...
    setOverview({
      ...
      topGrowthRegion: topGrowth ? (locale === 'ar' ? topGrowth.name_ar : topGrowth.name_en) : '-',
      topRevenueRegion: topRevenue ? (locale === 'ar' ? topRevenue.name_ar : topRevenue.name_en) : '-',
      totalOrders,
      totalRevenue,
    })
  }
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-sm text-slate-500">{locale === 'ar' ? 'المناطق النشطة' : 'Active Regions'}</p>
                  <p className="text-xl font-bold text-slate-900">{formatNumber(overview.activeRegions, locale)} / {formatNumber(overview.totalRegions, locale)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-sm text-slate-500">{locale === 'ar' ? 'أعلى نمو' : 'Top Growth'}</p>
                  <p className="text-lg font-bold text-slate-900 truncate">{overview.topGrowthRegion}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-sm text-slate-500">{locale === 'ar' ? 'أعلى إيرادات' : 'Top Revenue'}</p>
                  <p className="text-lg font-bold text-slate-900 truncate">{overview.topRevenueRegion}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-sm text-slate-500">{locale === 'ar' ? 'إجمالي الإيرادات' : 'Total Revenue'}</p>
                  <p className="text-lg font-bold text-slate-900">{formatCurrency(overview.totalRevenue, locale)} {locale === 'ar' ? 'ج.م' : 'EGP'}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="p-4 border-b border-slate-100">
              <div className="flex items-center justify-between">
                <h3 className="font-semibold text-slate-900 flex items-center gap-2">
                  {React.createElement(getViewLevelLabel(viewLevel).icon, { className: 'w-5 h-5 text-slate-600' })}
                  {getViewLevelLabel(viewLevel)[locale === 'ar' ? 'ar' : 'en']}
                  {getSelectedLocationName() && (
                    <span className="text-slate-500 font-normal">
                      {locale === 'ar' ? ' في ' : ' in '}{getSelectedLocationName()}
                    </span>
                  )}
                </h3>
              ...
              {regionStats.length > 0 ? (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center">
                        <span className="text-sm font-bold text-slate-600">{formatNumber(index + 1, locale)}</span>
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-4 text-sm text-slate-500">
                          <span className="flex items-center gap-1">
                            ...
                            {formatNumber(region.ordersCount, locale)} {locale === 'ar' ? 'طلب' : 'orders'}
                          </span>
                          <span className="flex items-center gap-1">
                            <Users className="w-3.5 h-3.5" />
                            {formatNumber(region.customersCount, locale)} {locale === 'ar' ? 'عميل' : 'customers'}
                          </span>
                          <span className="flex items-center gap-1">
                            <Store className="w-3.5 h-3.5" />
                            {formatNumber(region.providersCount, locale)} {locale === 'ar' ? 'متجر' : 'providers'}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="w-48 hidden lg:block">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs text-slate-500">{locale === 'ar' ? 'الإيرادات' : 'Revenue'}</span>
                          <span className="text-xs font-medium text-slate-700">{formatCurrency(region.revenue, locale)}</span>
                        </div>
                      ...
                      <div className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm font-medium ${
                        ...
                        {region.growthRate > 0 ? '+' : ''}{formatNumber(Math.round(region.growthRate * 10) / 10, locale)}%
                      </div>

                      {/* Avg Order Value */}
                      <div className="text-end hidden md:block">
                        <p className="text-sm text-slate-500">{locale === 'ar' ? 'متوسط الطلب' : 'Avg. Order'}</p>
                        <p className="font-medium text-slate-900">{formatCurrency(region.avgOrderValue, locale)} {locale === 'ar' ? 'ج.م' : 'EGP'}</p>
                      </div>
                    </div>
                  </div>
                ))
                ...
                <div className="p-12 text-center">
                  ...
                  <p className="text-slate-500">
                    {locale === 'ar' ? 'لا توجد بيانات للمناطق في هذه الفترة' : 'No region data for this period'}
                  </p>
                </div>
              )}
            </div>
                    ...
                    .map((region, index) => (
                      <div key={region.id} className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-slate-900">{locale === 'ar' ? region.name_ar : region.name_en}</span>
                        </div>
                        <span className="text-green-600 font-medium">+{formatNumber(Math.round(region.growthRate * 10) / 10, locale)}%</span>
                      </div>
                    ))}
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
                ...
                <div className="space-y-3">
                  ...
                  {regionStats
                    ...
                    .slice(0, 3)
                    .map((region) => (
                      <div key={region.id} className="p-3 bg-purple-50 rounded-lg border border-purple-100">
                        <div className="flex items-center gap-2 mb-1">
                  ...
                  {regionStats.filter(r => r.customersCount > 0 && r.providersCount < 3).length === 0 && (
