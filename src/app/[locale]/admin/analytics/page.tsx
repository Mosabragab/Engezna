  ...
  const loadAnalytics = useCallback(
    async (supabase: ReturnType<typeof createClient>) => {
      ...
      if (geoFilter.governorate_id) {
        providersForFilterQuery = providersForFilterQuery.eq(
          'governorate_id',
          geoFilter.governorate_id
        );
      }
      ...
      const avgOrderChange =
        prevAvgOrder > 0 ? ((avgOrderValue - prevAvgOrder) / prevAvgOrder) * 100 : 0;
      ...
      if (topProviderIds.length > 0) {
        ...
        const providers: TopProvider[] = (providersData || [])
          .map((p) => {
            ...
            {!isRegionalAdmin && !geoLoading && (
              <div className="flex items-center gap-3 lg:ml-auto pt-2 lg:pt-0 lg:border-l lg:border-slate-200 lg:pl-4">
                <span className="text-sm text-slate-500 whitespace-nowrap">
                  {locale === 'ar' ? 'فلترة جغرافية:' : 'Location:'}
                </span>
          ...
          <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              ...
              {overview.ordersChange !== 0 && (
                <div
                  className={`flex items-center gap-1 text-sm ${overview.ordersChange > 0 ? 'text-green-600' : 'text-red-600'}`}
                >
                  {overview.ordersChange > 0 ? (
            ...
            <p className="text-2xl font-bold text-slate-900">
              {formatNumber(overview.totalOrders, locale)}
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              ...
              {overview.revenueChange !== 0 && (
                <div
                  className={`flex items-center gap-1 text-sm ${overview.revenueChange > 0 ? 'text-green-600' : 'text-red-600'}`}
                >
                  {overview.revenueChange > 0 ? (
                    <ArrowUpRight className="w-4 h-4" />
                  ) : (
          ...
          <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              ...
              {overview.customersChange !== 0 && (
                <div
                  className={`flex items-center gap-1 text-sm ${overview.customersChange > 0 ? 'text-green-600' : 'text-red-600'}`}
                >
                  {overview.customersChange > 0 ? (
                    <ArrowUpRight className="w-4 h-4" />
                  ) : (
            ...
            <p className="text-2xl font-bold text-slate-900">
              {formatNumber(overview.newCustomers, locale)}
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              ...
              {overview.avgOrderChange !== 0 && (
                <div
                  className={`flex items-center gap-1 text-sm ${overview.avgOrderChange > 0 ? 'text-green-600' : 'text-red-600'}`}
                >
                  {overview.avgOrderChange > 0 ? (
                    <ArrowUpRight className="w-4 h-4" />
                  ) : (
            ...
            <p className="text-slate-500 text-sm mb-1">
              {locale === 'ar' ? 'متوسط قيمة الطلب' : 'Avg Order Value'}
            </p>
            <p className="text-2xl font-bold text-slate-900">
              {formatCurrency(overview.avgOrderValue, locale)} {locale === 'ar' ? 'ج.م' : 'EGP'}
            </p>
          </div>
        </div>
            ...
            <div className="space-y-3">
              {dailyStats.slice(-14).map((day) => (
                <div key={day.date} className="flex items-center gap-4">
                  ...
                  <span className="text-sm text-slate-600 w-16 text-end">
                    {formatNumber(day.orders, locale)} {locale === 'ar' ? 'طلب' : 'orders'}
                  </span>
                </div>
              ))}
            </div>
                  ...
                  <div key={provider.id} className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 text-sm text-slate-500">
                        <span>
                          {formatNumber(provider.orders_count, locale)}{' '}
                    ...
                    <div className="text-end">
                      <p className="font-bold text-slate-900">
                        {formatCurrency(provider.revenue, locale)}
                      </p>
                    ...
                    <div key={cat.category}>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-sm text-slate-600">
                          {formatCurrency(cat.revenue, locale)} {locale === 'ar' ? 'ج.م' : 'EGP'}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center justify-between mt-1">
                        <span className="text-xs text-slate-500">
                          {formatNumber(cat.orders, locale)} {locale === 'ar' ? 'طلب' : 'orders'}
                        </span>
                        <span className="text-xs text-slate-500">
                          {formatNumber(Math.round(percentage * 10) / 10, locale)}%
                        </span>
                      </div>
                    </div>
                  );
