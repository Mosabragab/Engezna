...
// Section type icons for offers page
...
// Section type labels for offers page
const sectionLabels: Record<string, { ar: string; en: string }> = {
  offers_hero: { ar: 'العرض المميز', en: 'Featured Offer' },
  promo_codes: { ar: 'أكواد الخصم', en: 'Promo Codes' },
  free_delivery: { ar: 'توصيل مجاني', en: 'Free Delivery' },
  flash_deals: { ar: 'عروض سريعة', en: 'Flash Deals' },
  category_offers: { ar: 'عروض الأقسام', en: 'Category Offers' },
};
...
export default function AdminOffersPage() {
  ...
  const handleCreatePreview = async () => {
    try {
      ...
      const url = `${window.location.origin}/${locale}/offers?preview=${token}`;
      setPreviewUrl(url);
  ...
  const handleSaveVersion = async () => {
    ...
    try {
      await saveLayoutVersion(`Offers Backup - ${new Date().toLocaleString(locale)}`);
      setSaveSuccess(true);
      setTimeout(() => setSaveSuccess(false), 3000);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <AdminHeader
        user={user}
        title={isRTL ? 'تخطيط صفحة العروض' : 'Offers Page Layout'}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                ...
                {isRTL ? 'إدارة صفحة العروض' : 'Offers Page Layout'}
              </h1>
              <p className="text-gray-600 mt-1">
                {isRTL
                  ? 'تحكم في ترتيب وإظهار أقسام صفحة العروض'
                  : 'Control the order and visibility of offers page sections'}
              </p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="p-4 border-b border-gray-200 bg-gray-50">
                <h2 className="font-semibold text-gray-900">
                  {isRTL ? 'أقسام صفحة العروض' : 'Offers Page Sections'}
                </h2>
              ...
              <div className="divide-y divide-gray-100">
                {localSections.length === 0 ? (
                  <div className="p-8 text-center text-gray-500">
                    ...
                    <p>
                      {isRTL
                        ? 'لا توجد أقسام. قم بتنفيذ migration قاعدة البيانات أولاً.'
                        : 'No sections found. Run the database migration first.'}
                    </p>
                  </div>
                ) : (
