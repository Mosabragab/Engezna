...
export default function AdminRolesPage() {
  ...
  async function loadData(supabase: ReturnType<typeof createClient>) {
    ...
    // Load actions
    const { data: actionsData } = await supabase.from('actions').select('*').order('code');
    ...
    try {
      if (editingRole) {
        ...
        if (selectedPermissions.size > 0) {
          ...
          const { error: permError } = await supabase.from('role_permissions').insert(permInserts);
  ...
  function isSomeResourceSelected(resourceId: string) {
    const resourcePerms = getPermissionsByResource(resourceId);
    return (
      resourcePerms.some((p) => selectedPermissions.has(p.id)) && !isAllResourceSelected(resourceId)
    );
  }
          ...
          <div className="bg-card-bg-success rounded-xl p-4 border border-success/30">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-sm text-success">
                {locale === 'ar' ? 'أدوار مخصصة' : 'Custom Roles'}
              </span>
            </div>
          ...
          <div className="bg-card-bg-purple rounded-xl p-4 border border-purple/30">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-sm text-purple">
                {locale === 'ar' ? 'إجمالي الصلاحيات' : 'Total Permissions'}
              </span>
            </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder={
                  locale === 'ar' ? 'بحث بالاسم أو الكود...' : 'Search by name or code...'
                }
                value={searchQuery}
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="ShieldCheck">
                        {locale === 'ar' ? 'درع تحقق' : 'Shield Check'}
                      </option>
                      ...
                      <option value="ShoppingCart">
                        {locale === 'ar' ? 'عربة تسوق' : 'Shopping Cart'}
                      </option>
                      ...
                      >
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs text-slate-500">({resourcePerms.length})</span>
                          </div>
                          {isExpanded ? (
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full p-6 my-8 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <h3 className="font-semibold text-slate-900">
                {locale === 'ar' ? 'الصلاحيات' : 'Permissions'} (
                {viewLoading ? '...' : rolePermissions.length})
              </h3>
              ...
              {viewLoading ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-slate-500">
                    {locale === 'ar' ? 'جاري تحميل الصلاحيات...' : 'Loading permissions...'}
                  </p>
                </div>
              ) : (
                <>
