...
const defaultSettings: PlatformSettings = {
  ...
  // Commission settings - 0% grace period, max 7% after (will be updated to 30 days in Phase 0B)
  default_commission_rate: 7.0,
  max_commission_rate: 7.0,
  grace_period_days: 90, // Legacy 90 — Phase 0B will change to 30
...
export default function AdminSettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    'general' | 'commission' | 'payments' | 'delivery' | 'notifications' | 'account'
  >('general');
  ...
  async function handleChangePassword() {
    ...
    if (newPassword !== confirmPassword) {
      setPasswordError(
        locale === 'ar' ? 'كلمة المرور الجديدة غير متطابقة' : 'New passwords do not match'
      );
      return;
    }
    ...
    if (signInError) {
      setPasswordError(
        locale === 'ar' ? 'كلمة المرور الحالية غير صحيحة' : 'Current password is incorrect'
      );
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-yellow-700">
                    {locale === 'ar'
                      ...
                      : 'When enabled, users cannot access the platform'}
                  </p>
                </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSettings({
                          ...settings,
                          max_commission_rate: parseFloat(e.target.value) || 0,
                        })
                      }
                      className={`w-full ${isRTL ? 'pr-10 pl-4' : 'pl-10 pr-4'} py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-red-500`}
                      min="0"
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    {locale === 'ar'
                      ? `فترة السماح (بالأيام). حاليًا: ${settings.grace_period_days} يوم`
                      : `Grace period (in days). Currently: ${settings.grace_period_days} days`}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setSettings({
                        ...settings,
                        provider_commission_payout_day: parseInt(e.target.value),
                      })
                    }
                    className="w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-red-500"
                  >
              ...
              <div className="flex items-center gap-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setSettings({ ...settings, grace_period_enabled: e.target.checked })
                    }
                    className="sr-only peer"
                  />
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSettings({
                          ...settings,
                          platform_commission_rate: parseFloat(e.target.value) || 0,
                        })
                      }
                      className={`w-full ${isRTL ? 'pr-10 pl-4' : 'pl-10 pr-4'} py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-red-500`}
                      min="0"
                ...
                <div>
                  ...
                  >
                    <option value="EGP">
                      {locale === 'ar' ? 'جنيه مصري (EGP)' : 'Egyptian Pound (EGP)'}
                    </option>
                    <option value="USD">
                      {locale === 'ar' ? 'دولار أمريكي (USD)' : 'US Dollar (USD)'}
                    </option>
                    <option value="SAR">
                      {locale === 'ar' ? 'ريال سعودي (SAR)' : 'Saudi Riyal (SAR)'}
                    </option>
                  </select>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <label className="flex items-center gap-3 p-4 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50">
                    <input
                      ...
                      onChange={(e) =>
                        setSettings({ ...settings, allow_cash_payment: e.target.checked })
                      }
                      className="w-4 h-4 text-red-600 border-slate-300 rounded focus:ring-red-500"
                    />
                    <span className="text-slate-700">
                      {locale === 'ar' ? 'الدفع النقدي' : 'Cash Payment'}
                    </span>
                  </label>
                  <label className="flex items-center gap-3 p-4 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50">
                    <input
                      ...
                      onChange={(e) =>
                        setSettings({ ...settings, allow_card_payment: e.target.checked })
                      }
                      className="w-4 h-4 text-red-600 border-slate-300 rounded focus:ring-red-500"
                    />
                    <span className="text-slate-700">
                      {locale === 'ar' ? 'البطاقة البنكية' : 'Card Payment'}
                    </span>
                  </label>
                  <label className="flex items-center gap-3 p-4 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50">
                    <input
                      ...
                      onChange={(e) =>
                        setSettings({ ...settings, allow_wallet_payment: e.target.checked })
                      }
                      className="w-4 h-4 text-red-600 border-slate-300 rounded focus:ring-red-500"
                    />
                    <span className="text-slate-700">
                      {locale === 'ar' ? 'المحفظة الإلكترونية' : 'E-Wallet'}
                    </span>
                  </label>
                </div>
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSettings({
                          ...settings,
                          delivery_fee_per_km: parseFloat(e.target.value) || 0,
                        })
                      }
                      className={`w-full ${isRTL ? 'pr-10 pl-4' : 'pl-10 pr-4'} py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-red-500`}
                      min="0"
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSettings({
                          ...settings,
                          max_delivery_distance_km: parseFloat(e.target.value) || 0,
                        })
                      }
                      className={`w-full ${isRTL ? 'pr-10 pl-4' : 'pl-10 pr-4'} py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-red-500`}
                      min="0"
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">
                    {locale === 'ar'
                      ? 'وقت إلغاء الطلب التلقائي (دقيقة)'
                      : 'Auto-Cancel Time (minutes)'}
                  </label>
                  <div className="relative">
                    <Clock
                      className={`absolute ${isRTL ? 'right-3' : 'left-3'} top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400`}
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSettings({
                          ...settings,
                          order_auto_cancel_minutes: parseInt(e.target.value) || 0,
                        })
                      }
                      className={`w-full ${isRTL ? 'pr-10 pl-4' : 'pl-10 pr-4'} py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-red-500`}
                      min="0"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder={
                          locale === 'ar' ? 'أدخل كلمة المرور الحالية' : 'Enter current password'
                        }
                      />
                      <button
                        type="button"
