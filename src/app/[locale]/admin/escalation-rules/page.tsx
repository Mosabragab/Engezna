...
const ACTION_TYPES = [
  { value: 'require_approval', label: { ar: 'يتطلب موافقة', en: 'Require Approval' }, icon: Lock, color: 'amber' },
...
export default function EscalationRulesPage() {
  ...
  async function handleDelete(ruleId: string) {
    if (!confirm(locale === 'ar' ? 'هل أنت متأكد من حذف هذه القاعدة؟' : 'Are you sure you want to delete this rule?')) {
      return
    }
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{locale === 'ar' ? 'إجمالي القواعد' : 'Total Rules'}</p>
                </div>
              </div>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{locale === 'ar' ? 'قواعد نشطة' : 'Active Rules'}</p>
                </div>
              </div>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{locale === 'ar' ? 'تتطلب موافقة' : 'Require Approval'}</p>
                </div>
              </div>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{locale === 'ar' ? 'قواعد المنع' : 'Block Rules'}</p>
                </div>
              </div>
            </div>
          </div>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap items-center gap-2 mt-2">
                            ...
                            {rule.trigger_conditions.resource && (
                              <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded">
                                {RESOURCES.find(r => r.value === rule.trigger_conditions.resource)?.[`label`][locale === 'ar' ? 'ar' : 'en'] || rule.trigger_conditions.resource}
                              </span>
                            )}
                            {rule.trigger_conditions.action && (
                              <span className="text-xs bg-purple-100 text-purple-700 px-2 py-1 rounded">
                                {ACTIONS.find(a => a.value === rule.trigger_conditions.action)?.[`label`][locale === 'ar' ? 'ar' : 'en'] || rule.trigger_conditions.action}
                              </span>
                            )}
                            ...
                            {rule.trigger_conditions.amount && (
                              <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded">
                                {locale === 'ar' ? '>' : '>'} {rule.trigger_conditions.amount} {locale === 'ar' ? 'ج.م' : 'EGP'}
                              </span>
                            )}
                            {rule.trigger_conditions.count_per_day && (
                              <span className="text-xs bg-orange-100 text-orange-700 px-2 py-1 rounded">
                                {locale === 'ar' ? '>' : '>'} {rule.trigger_conditions.count_per_day} {locale === 'ar' ? '/يوم' : '/day'}
                              </span>
                            )}

                            {/* Action Type */}
                            <span className={`inline-flex items-center gap-1 text-xs px-2 py-1 rounded ${
                              actionType?.color === 'amber' ? 'bg-amber-100 text-amber-700' :
                              actionType?.color === 'blue' ? 'bg-blue-100 text-blue-700' :
                          ...
                          {rule.escalate_to_role && (
                            <div className="mt-2 flex items-center gap-1.5 text-sm text-slate-600">
                              ...
                              >
                                {locale === 'ar' ? rule.escalate_to_role.name_ar : rule.escalate_to_role.name_en}
                              </span>
                            </div>
                          )}
                        </div>
            ...
            <div className="text-center py-12 bg-white rounded-xl border border-slate-200">
              ...
              <p className="text-slate-500">
                {searchQuery
                  ...
                  : (locale === 'ar' ? 'ابدأ بإنشاء قاعدة جديدة' : 'Start by creating a new rule')}
              </p>
            </div>
          )}
      </main>
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full p-6 my-8 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-slate-900">
                {editingRule
                  ? (locale === 'ar' ? 'تعديل القاعدة' : 'Edit Rule')
                  : (locale === 'ar' ? 'إنشاء قاعدة جديدة' : 'Create New Rule')}
              </h2>
            ...
            <div className="space-y-4">
              ...
              {formData.trigger_type === 'count' && (
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, count_per_day: Number(e.target.value) })}
                    placeholder="5"
                    className="w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-red-500"
                  />
                </div>
                  ...
                  {ACTION_TYPES.map((type) => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          borderColor: formData.action_type === type.value
                            ? (type.color === 'amber' ? '#f59e0b' : type.color === 'blue' ? '#3b82f6' : '#ef4444')
                            : undefined,
                          backgroundColor: formData.action_type === type.value
                            ? (type.color === 'amber' ? '#fef3c7' : type.color === 'blue' ? '#dbeafe' : '#fee2e2')
                            : undefined,
                        }}
                      >
                        <Icon className="w-5 h-5 mx-auto mb-1" style={{
                          color: formData.action_type === type.value
                            ? (type.color === 'amber' ? '#d97706' : type.color === 'blue' ? '#2563eb' : '#dc2626')
                            : '#94a3b8'
                        }} />
                        <p className="text-xs font-medium" style={{
                          color: formData.action_type === type.value
                            ? (type.color === 'amber' ? '#b45309' : type.color === 'blue' ? '#1d4ed8' : '#b91c1c')
                            : '#475569'
                        }}>
                          {locale === 'ar' ? type.label.ar : type.label.en}
                        </p>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, escalate_to_role_id: e.target.value })}
                  className="w-full px-4 py-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-red-500"
                >
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  {locale === 'ar' ? 'الأرقام الأصغر تُنفذ أولاً' : 'Lower numbers are executed first'}
                </p>
              </div>
            </div>

            <div className="flex gap-3 mt-6 pt-4 border-t">
              <Button variant="outline" onClick={() => setShowModal(false)} className="flex-1" disabled={formLoading}>
                {locale === 'ar' ? 'إلغاء' : 'Cancel'}
              </Button>
              <Button onClick={handleSave} className="flex-1 bg-red-600 hover:bg-red-700" disabled={formLoading}>
                {formLoading ? (
