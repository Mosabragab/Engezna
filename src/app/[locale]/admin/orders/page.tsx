    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(
        (o) =>
          ...
          (locale === 'ar' ? o.provider?.name_ar : o.provider?.name_en)
            ?.toLowerCase()
            .includes(query)
      );
    }
        ...
        <div className="grid grid-cols-2 lg:grid-cols-6 gap-4 mb-6">
          ...
          <div className="bg-card-bg-success rounded-xl p-4 border border-success/30">
            ...
            <p className="text-2xl font-bold text-success">
              {formatNumber(stats.delivered, locale)}
            </p>
          </div>
          ...
          <div className="bg-card-bg-emerald rounded-xl p-4 border border-emerald/30">
            ...
            <p className="text-xl font-bold text-emerald">
              {formatCurrency(stats.totalRevenue, locale)} {locale === 'ar' ? 'ج.م' : 'EGP'}
            </p>
          </div>
        </div>
                ...
                {filteredOrders.length > 0 ? (
                  filteredOrders.map((order) => (
                    <tr key={order.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center gap-1.5 text-xs px-2 py-1 rounded-full ${getStatusColor(order.status)}`}
                        >
                      ...
                      <td className="px-4 py-3">
                        <div>
                          ...
                          <span
                            className={`text-xs ${order.payment_status === 'completed' ? 'text-green-600' : 'text-yellow-600'}`}
                          >
