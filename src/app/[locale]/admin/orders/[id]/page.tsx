        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">{formatDateTime(order.created_at, locale)}</p>
            </div>
          </div>
                    ...
                    <div key={item.id} className="p-4 flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-slate-500">
                          {formatCurrency(parseFloat(item.unit_price), locale)} x{' '}
                      ...
                      <p className="font-semibold text-slate-900">
                        {formatCurrency(parseFloat(item.total_price), locale)}{' '}
                        {locale === 'ar' ? 'ج.م' : 'EGP'}
                      </p>
                    </div>
                  ))
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-slate-900">
                      {formatCurrency(order.subtotal, locale)} {locale === 'ar' ? 'ج.م' : 'EGP'}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-600">
                    ...
                    <span className="text-slate-900">
                      {formatCurrency(order.delivery_fee, locale)} {locale === 'ar' ? 'ج.م' : 'EGP'}
                    </span>
                  </div>
                  {order.discount > 0 && (
                    <div className="flex justify-between text-sm text-green-600">
                      ...
                      <span>
                        -{formatCurrency(order.discount, locale)} {locale === 'ar' ? 'ج.م' : 'EGP'}
                      </span>
                    </div>
                  )}
                    ...
                    <div className="text-end">
                      {order.platform_commission === 0 && (order.original_commission || 0) > 0 ? (
                        ...
                        <span className="text-red-600">
                          {formatCurrency(order.platform_commission, locale)}{' '}
                          {locale === 'ar' ? 'ج.م' : 'EGP'}
                        </span>
                      )}
                    </div>
                      ...
                      <div key={refund.id} className="p-4 space-y-3">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-semibold text-amber-800">
                              {formatCurrency(refund.processed_amount || refund.amount, locale)}{' '}
                              {locale === 'ar' ? 'ج.م' : 'EGP'}
                            </span>
                          </div>
                          <span
                        ...
                        <div className="flex flex-wrap gap-2 text-xs">
                          ...
                          {refund.provider_action && (
                            <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded">
                              {refund.provider_action === 'accept_full'
                                ? locale === 'ar'
                                  ? 'قبول كامل'
                                  : 'Full Accept'
                                : refund.provider_action === 'accept_partial'
                                  ? locale === 'ar'
                                    ? 'قبول جزئي'
                                    : 'Partial Accept'
                                  : refund.provider_action === 'reject'
                                    ? locale === 'ar'
                                      ? 'رفض'
                                      : 'Rejected'
                                    : refund.provider_action}
                        ...
                        {(refund.review_notes || refund.processing_notes) && (
                          <div className="text-sm bg-slate-50 p-2 rounded">
                            ...
                            <p className="text-slate-700 mt-1">
                              {refund.processing_notes || refund.review_notes}
                            </p>
                          </div>
                        )}
                        ...
                        <div className="text-xs text-slate-500 space-y-1">
                          <div className="flex items-center gap-1">
                            ...
                            {formatDateTime(refund.created_at, locale)}
                          </div>
                          {refund.reviewed_at && (
                            <div className="flex items-center gap-1">
                              <CheckCircle2 className="w-3 h-3" />
                              {locale === 'ar' ? 'تاريخ المراجعة:' : 'Reviewed:'}{' '}
                              {formatDateTime(refund.reviewed_at, locale)}
                            </div>
                          )}
                          {refund.processed_at && (
                            <div className="flex items-center gap-1">
                              <DollarSign className="w-3 h-3" />
                              {locale === 'ar' ? 'تاريخ التنفيذ:' : 'Processed:'}{' '}
                              {formatDateTime(refund.processed_at, locale)}
                            </div>
                          )}
                        </div>
                      </div>
          ...
          <div className="space-y-6">
              ...
              <div className="p-4">
                {order.customer ? (
                    ...
                    {order.customer.email && (
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <a
                          ...
                          className="text-slate-600 hover:text-red-600 truncate"
                        >
                          {order.customer.email}
                        </a>
                      </div>
                  ...
                  <p className="text-slate-500">
                    {locale === 'ar' ? 'لا توجد معلومات' : 'No info'}
                  </p>
                )}
              </div>
            </div>
              ...
              <div className="p-4">
                {order.delivery_address ? (
                  ...
                  <p className="text-slate-500 text-sm">
                    {locale === 'ar' ? 'لم يتم تحديد عنوان' : 'No address specified'}
                  </p>
                )}
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">
                    {locale === 'ar' ? 'حالة الدفع' : 'Status'}
                  </span>
                  <span
                    className={`font-medium ${order.payment_status === 'completed' ? 'text-green-600' : 'text-yellow-600'}`}
                  >
