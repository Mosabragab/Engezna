...
export default function BroadcastAnalyticsPage() {
  ...
  const [sortBy, setSortBy] = useState<'win_rate' | 'total_requests' | 'avg_response_time'>('win_rate')
  ...
  const loadAnalytics = useCallback(async () => {
    ...
    try {
        ...
        if (pricedRequests.length > 0) {
          ...
          const timeToFirstPrice =
            (new Date(firstPriced.responded_at).getTime() -
              new Date(broadcast.created_at).getTime()) / 60000
      ...
      const calculatedMetrics: BroadcastMetrics = {
        ...
        avg_time_to_first_price: countWithFirstPrice > 0 ? totalTimeToFirstPrice / countWithFirstPrice : 0,
        avg_time_to_selection: countWithSelection > 0 ? totalTimeToSelection / countWithSelection : 0,
      ...
      const providerStatsArray: ProviderStats[] = Array.from(providerMap.values()).map((stats) => ({
        ...
        avg_response_time: stats.responseCount > 0 ? stats.totalResponseTime / stats.responseCount : 0,
  ...
  const exportCSV = () => {
    const headers = ['Provider', 'Total Requests', 'Priced', 'Won', 'Win Rate %', 'Avg Response (min)', 'Total Revenue', 'Avg Order Value']
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              ...
              >
                ...
                <p className="text-2xl font-bold text-slate-800 dark:text-slate-200">
                  ...
                  <span className="text-sm font-normal text-slate-400 ms-1">/ {metrics?.total_broadcasts || 0}</span>
                </p>
                <p className="text-xs text-slate-500 dark:text-slate-400">{isRTL ? 'البث المكتمل' : 'Completed Broadcasts'}</p>
              </motion.div>
              ...
              >
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">{isRTL ? 'متوسط وقت أول تسعير' : 'Avg Time to First Quote'}</p>
              </motion.div>
              ...
              >
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">{isRTL ? 'متوسط وقت الاختيار' : 'Avg Time to Selection'}</p>
              </motion.div>
              ...
              >
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">{isRTL ? 'إجمالي قيمة الطلبات' : 'Total Order Value'}</p>
              </motion.div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-lg font-bold text-slate-800 dark:text-slate-200">{metrics?.expired_broadcasts || 0}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">{isRTL ? 'منتهي الصلاحية' : 'Expired'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-lg font-bold text-slate-800 dark:text-slate-200">{metrics?.avg_providers_per_broadcast.toFixed(1) || 0}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">{isRTL ? 'متوسط التجار/بث' : 'Avg Providers/Broadcast'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-lg font-bold text-slate-800 dark:text-slate-200">{dailyStats.length}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">{isRTL ? 'أيام نشطة' : 'Active Days'}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      <option value="total_requests">{isRTL ? 'إجمالي الطلبات' : 'Total Requests'}</option>
                      <option value="avg_response_time">{isRTL ? 'وقت الاستجابة' : 'Response Time'}</option>
                    </select>
                    <Button
                      variant="ghost"
                      size="icon"
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-slate-50 dark:bg-slate-900/50">
                    <tr>
                      <th className="text-start px-4 py-3 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">#</th>
                      <th className="text-start px-4 py-3 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{isRTL ? 'التاجر' : 'Provider'}</th>
                      <th className="text-center px-4 py-3 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{isRTL ? 'الطلبات' : 'Requests'}</th>
                      <th className="text-center px-4 py-3 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{isRTL ? 'المسعرة' : 'Priced'}</th>
                      <th className="text-center px-4 py-3 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{isRTL ? 'الفائزة' : 'Won'}</th>
                      <th className="text-center px-4 py-3 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{isRTL ? 'نسبة الفوز' : 'Win Rate'}</th>
                      <th className="text-center px-4 py-3 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{isRTL ? 'وقت الاستجابة' : 'Resp. Time'}</th>
                      <th className="text-end px-4 py-3 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">{isRTL ? 'الإيرادات' : 'Revenue'}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                    {sortedProviders.map((provider, index) => (
                      <tr key={provider.provider_id} className="hover:bg-slate-50 dark:hover:bg-slate-900/30">
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-center w-6 h-6">
                            ...
                            {index > 2 && <span className="text-sm text-slate-500">{index + 1}</span>}
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center overflow-hidden">
                              {provider.provider_logo ? (
                                ...
                                <span className="text-sm font-medium text-slate-600 dark:text-slate-400">{provider.provider_name.charAt(0)}</span>
                              )}
                            </div>
                            <span className="font-medium text-slate-800 dark:text-slate-200">{provider.provider_name}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-center text-slate-600 dark:text-slate-400">{provider.total_requests}</td>
                        <td className="px-4 py-3 text-center text-slate-600 dark:text-slate-400">{provider.priced_requests}</td>
                        <td className="px-4 py-3 text-center font-medium text-emerald-600 dark:text-emerald-400">{provider.won_requests}</td>
                        <td className="px-4 py-3 text-center">
                          <div className="flex items-center justify-center gap-1">
                            <span className={`font-bold ${
                              provider.win_rate >= 50 ? 'text-emerald-600 dark:text-emerald-400' :
                              provider.win_rate >= 25 ? 'text-amber-600 dark:text-amber-400' :
                              'text-slate-600 dark:text-slate-400'
                            }`}>{provider.win_rate.toFixed(1)}%</span>
                            {provider.win_rate >= 50 && <Star className="w-3 h-3 text-amber-500 fill-current" />}
                          </div>
                        </td>
                        <td className="px-4 py-3 text-center">
                          <div className="flex items-center justify-center gap-1">
                            ...
                            <span className="text-sm text-slate-600 dark:text-slate-400">{formatTime(provider.avg_response_time)}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-end font-medium text-slate-800 dark:text-slate-200">{formatCurrency(provider.total_revenue)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {sortedProviders.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-slate-500 dark:text-slate-400">{isRTL ? 'لا توجد بيانات للتجار في هذه الفترة' : 'No provider data for this period'}</p>
                </div>
              )}
            </motion.div>
            ...
            >
              ...
              {dailyStats.length > 0 ? (
                <div className="space-y-3">
                  <div className="flex items-end gap-1 h-40">
                    {dailyStats.slice(-14).map((day) => {
                      ...
                      const completionRate = day.broadcasts > 0 ? (day.completed / day.broadcasts) * 100 : 0
                  ...
                  <div className="flex items-center justify-center gap-6 pt-6">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-slate-500 dark:text-slate-400">{isRTL ? 'إجمالي البث' : 'Total Broadcasts'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 rounded-full bg-emerald-500" />
                      <span className="text-xs text-slate-500 dark:text-slate-400">{isRTL ? 'نسبة إتمام عالية' : 'High Completion'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 rounded-full bg-amber-500" />
                      <span className="text-xs text-slate-500 dark:text-slate-400">{isRTL ? 'نسبة إتمام متوسطة' : 'Medium Completion'}</span>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="text-center py-12">
                  ...
                  <p className="text-slate-500 dark:text-slate-400">{isRTL ? 'لا توجد بيانات يومية' : 'No daily data available'}</p>
                </div>
              )}
            </motion.div>
          </>
