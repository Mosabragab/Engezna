...
export default function SupervisorPermissionsPage({ params }: PageProps) {
  ...
  async function handleRemoveRole(roleId: string) {
    if (!confirm(locale === 'ar' ? 'هل أنت متأكد من إزالة هذا الدور؟' : 'Are you sure you want to remove this role?')) {
      return
    }
  ...
  async function handleRemoveDirectPermission(permissionId: string) {
    if (!confirm(locale === 'ar' ? 'هل أنت متأكد من إزالة هذه الصلاحية؟' : 'Are you sure you want to remove this permission?')) {
      return
    }
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-2">
                  <span className={`inline-flex items-center gap-1 text-xs px-2 py-1 rounded-full ${
                    ...
                    {supervisor.is_active ? <CheckCircle2 className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                    ...
                    >
                      ...
                      {ar.expires_at && (
                        <div className="mt-2 flex items-center gap-1 text-xs text-amber-600">
                          ...
                          {locale === 'ar' ? 'ينتهي:' : 'Expires:'} {new Date(ar.expires_at).toLocaleDateString()}
                        </div>
                      )}
                    </div>
                  ))
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className={`font-medium ${ap.grant_type === 'grant' ? 'text-green-700' : 'text-red-700'}`}>
                              {ap.permission?.code}
                            </p>
                            <p className="text-xs text-slate-500">
                              {ap.grant_type === 'grant'
                                ? (locale === 'ar' ? 'صلاحية إضافية' : 'Extra permission')
                                : (locale === 'ar' ? 'صلاحية ممنوعة' : 'Denied permission')}
                            </p>
                          </div>
                        </div>
                        <Button
              ...
              <div className="space-y-2">
                {resourceCodes.map(resourceCode => {
                  ...
                  const grantedCount = resourcePerms.filter(p =>
                    adminPermissions.some(ap => ap.permission_id === p.id && ap.grant_type === 'grant')
                  ).length

                  const deniedCount = resourcePerms.filter(p =>
                    adminPermissions.some(ap => ap.permission_id === p.id && ap.grant_type === 'deny')
                  ).length

                  return (
                    <div key={resourceCode} className="border border-slate-200 rounded-lg overflow-hidden">
                      <button
                      ...
                      {isExpanded && (
                        <div className="p-3 grid gap-2 sm:grid-cols-3">
                          {resourcePerms.map(perm => {
                            const directPerm = adminPermissions.find(ap => ap.permission_id === perm.id)
                            ...
                            return (
                              ...
                              >
                                ...
                                <span>{locale === 'ar' ? actionConfig?.label.ar : actionConfig?.label.en}</span>
                              </div>
                            )
                          })}
                        </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
            ...
            <div className="flex gap-3 mt-6">
              ...
              >
                {saving ? <RefreshCw className="w-4 h-4 animate-spin" /> : (locale === 'ar' ? 'إضافة' : 'Add')}
              </Button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full p-6 my-8 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowAddPermissionModal(false)} className="text-slate-400 hover:text-slate-600">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-3">
                  ...
                  >
                    <CheckCircle2 className={`w-6 h-6 mx-auto mb-1 ${newPermissionType === 'grant' ? 'text-green-600' : 'text-slate-400'}`} />
                    <p className={`text-sm font-medium ${newPermissionType === 'grant' ? 'text-green-700' : 'text-slate-600'}`}>
                  ...
                  >
                    <XCircle className={`w-6 h-6 mx-auto mb-1 ${newPermissionType === 'deny' ? 'text-red-600' : 'text-slate-400'}`} />
                    <p className={`text-sm font-medium ${newPermissionType === 'deny' ? 'text-red-700' : 'text-slate-600'}`}>
              ...
              <div>
                ...
                >
                  <option value="">{locale === 'ar' ? 'اختر...' : 'Select...'}</option>
                  {resourceCodes.map(resourceCode => {
                    const perms = availablePermissions.filter(p => p.resource_code === resourceCode)
                    if (perms.length === 0) return null
                    const config = RESOURCE_CONFIG[resourceCode as ResourceCode]
                    return (
                      <optgroup key={resourceCode} label={locale === 'ar' ? config?.label.ar : config?.label.en}>
                        {perms.map(perm => (
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">
                  {locale === 'ar' ? 'السبب' : 'Reason'} <span className="text-slate-400 text-xs">({locale === 'ar' ? 'اختياري' : 'optional'})</span>
                </label>
                <textarea
                  value={newPermissionReason}
                  onChange={(e) => setNewPermissionReason(e.target.value)}
                  placeholder={locale === 'ar' ? 'سبب منح أو منع هذه الصلاحية...' : 'Reason for granting or denying this permission...'}
                  rows={2}
              ...
              {newPermissionType === 'grant' && selectedPermissionToAdd && (
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">
                    {locale === 'ar' ? 'القيود' : 'Constraints'} <span className="text-slate-400 text-xs">({locale === 'ar' ? 'اختياري' : 'optional'})</span>
                  </label>
                  {(() => {
            ...
            <div className="flex gap-3 mt-6">
              <Button variant="outline" onClick={() => setShowAddPermissionModal(false)} className="flex-1">
                {locale === 'ar' ? 'إلغاء' : 'Cancel'}
              </Button>
              <Button
              ...
              >
                {saving ? (
                  ...
                  <>
                    {newPermissionType === 'grant' ? <Plus className="w-4 h-4 me-1" /> : <Minus className="w-4 h-4 me-1" />}
                    {newPermissionType === 'grant'
                      ? (locale === 'ar' ? 'منح الصلاحية' : 'Grant Permission')
                      : (locale === 'ar' ? 'منع الصلاحية' : 'Deny Permission')}
                  </>
                )}
              </Button>
            </div>
