...
export default function AdminMessagesPage() {
  ...
  const calculateStats = useCallback((data: Message[], adminId: string | null) => {
    ...
    const inbox = data.filter((m) => m.recipient_ids?.includes(adminId) || m.is_broadcast).length;

    const unread = data.filter(
      (m) => (m.recipient_ids?.includes(adminId) || m.is_broadcast) && !m.read_by?.includes(adminId)
    ).length;
  ...
  const filterMessages = useCallback(() => {
    ...
    if (viewMode === 'inbox') {
      filtered = filtered.filter(
        (m) => m.recipient_ids?.includes(currentAdminId) || m.is_broadcast
      );
  ...
  async function handleSendMessage() {
    ...
    if (!currentAdminId) {
      setFormError(
        locale === 'ar'
          ? 'لم يتم التعرف على حسابك. يرجى إعادة تسجيل الدخول.'
          : 'Your account was not recognized. Please log in again.'
      );
      return;
    }
    ...
    if (!formData.is_broadcast && formData.recipients.length === 0) {
      setFormError(
        locale === 'ar' ? 'اختر مستلماً واحداً على الأقل' : 'Select at least one recipient'
      );
      return;
    }
    ...
    const notifications = recipients.map((recipientId) => ({
      ...
      type: 'message',
      title: locale === 'ar' ? `رسالة جديدة من ${senderName}` : `New message from ${senderName}`,
      body:
        formData.subject ||
        (formData.body.length > 100 ? formData.body.substring(0, 100) + '...' : formData.body),
  ...
  async function markAsRead(message: Message) {
    ...
    // Update local state
    setMessages((prev) =>
      prev.map((m) => (m.id === message.id ? { ...m, read_by: newReadBy } : m))
    );
  }
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-medium text-yellow-800">
                  {locale === 'ar'
                    ? 'تحذير: مشكلة في حالة الحساب'
                    : 'Warning: Account Status Issue'}
                </h3>
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between gap-2 mb-1">
                                <span
                                  className={`font-medium ${unread ? 'text-slate-900' : 'text-slate-700'}`}
                                >
                                  {viewMode === 'sent'
                                    ? (locale === 'ar' ? 'إلى: ' : 'To: ') +
                                      (message.is_broadcast
                                        ? locale === 'ar'
                                          ? 'الجميع'
                                          : 'Everyone'
                                        : `${message.recipient_ids?.length || 0} ${locale === 'ar' ? 'مستلم' : 'recipients'}`)
                              ...
                              <p
                                className={`text-sm ${unread ? 'font-medium text-slate-900' : 'text-slate-600'} truncate`}
                              >
