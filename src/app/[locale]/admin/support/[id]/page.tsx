...
export default function AdminSupportTicketDetailPage() {
      ...
      return allMessages.filter(
        (msg) =>
          ...
          (msg.sender_type === 'admin' &&
            (!msg.recipient_type || msg.recipient_type === 'customer'))
      );
    } else {
  ...
  const customerMessagesCount = allMessages.filter(
    (msg) =>
      msg.sender_type === 'customer' ||
      (msg.sender_type === 'admin' && (!msg.recipient_type || msg.recipient_type === 'customer'))
  ).length;

  const providerMessagesCount = allMessages.filter(
    (msg) =>
      msg.sender_type === 'provider' ||
      (msg.sender_type === 'admin' && msg.recipient_type === 'provider')
  ).length;
  ...
  async function handleSendMessage() {
    ...
    const result = await supabase.from('ticket_messages').insert({
      ticket_id: ticketId,
      sender_id: user.id,
      sender_type: 'admin',
    ...
    if (result.error) {
      ...
      if (result.error.message?.includes('recipient_type')) {
        const fallbackResult = await supabase.from('ticket_messages').insert({
          ticket_id: ticketId,
          sender_id: user.id,
          sender_type: 'admin',
          message: newMessage.trim(),
  ...
  return (
    <>
      <AdminHeader user={user} title={`#${ticket.ticket_number}`} onMenuClick={toggleSidebar} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
              <div
                className={`flex items-start justify-between gap-4 mb-4 ${isRTL ? 'flex-row-reverse' : ''}`}
              >
                ...
                <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <span
                    className={`inline-flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-full border ${getStatusColor(ticket.status)}`}
                  >
              ...
              {/* Action Buttons */}
              <div
                className={`flex items-center gap-2 pt-3 border-t border-slate-100 ${isRTL ? 'flex-row-reverse' : ''}`}
              >
                {ticket.status === 'open' && (
                  <Button
                    size="sm"
                  ...
                  >
                    {actionLoading ? (
                      <RefreshCw className={`w-4 h-4 animate-spin ${isRTL ? 'ml-1' : 'mr-1'}`} />
                    ) : (
                ...
                <Button size="sm" variant="outline" onClick={loadTicket}>
            ...
            {/* Chat Tabs - Only show for non-contact form tickets */}
            {ticket.source !== 'contact_form' && (
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                {/* Tab Headers */}
                <div
                  className={`flex border-b border-slate-200 ${isRTL ? 'flex-row-reverse' : ''}`}
                >
                  <button
                  ...
                  {filteredMessages.length === 0 ? (
                    ...
                    filteredMessages.map((msg) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`max-w-[80%] rounded-2xl px-4 py-2.5 shadow-sm ${messageColor}`}
                          >
          ...
          <div className="space-y-4">
            {/* Contact Form Sender Info - Show for contact form tickets */}
            {ticket.source === 'contact_form' ? (
              <div className="bg-purple-50 rounded-xl border border-purple-200 shadow-sm p-4">
                <h3
                  className={`font-semibold text-purple-900 mb-3 flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}
                >
                  <Mail className="w-5 h-5" />
                  {locale === 'ar' ? 'معلومات المرسل' : 'Sender Info'}
                </h3>
                <div className="space-y-3">
                  <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={isRTL ? 'text-right' : ''}>
                      <p className="font-medium text-purple-900">{ticket.contact_name || '-'}</p>
                      <p className="text-xs text-purple-600">
                        {locale === 'ar' ? 'من صفحة التواصل' : 'From Contact Form'}
                      </p>
                    </div>
                  </div>
                  {ticket.contact_email && (
                    <div
                      className={`flex items-center gap-2 text-sm text-purple-700 ${isRTL ? 'flex-row-reverse' : ''}`}
                    >
                      <Mail className="w-4 h-4" />
                      <a href={`mailto:${ticket.contact_email}`} className="hover:underline">
                        {ticket.contact_email}
                      </a>
                    </div>
                  )}
                  {ticket.contact_phone && (
                    ...
                    >
                      ...
                      <span dir="ltr">{ticket.contact_phone}</span>
                    </div>
                  )}
                  {/* WhatsApp Button */}
                  {ticket.contact_phone && (
                    <a
                      href={`https://wa.me/${ticket.contact_phone.replace(/[^0-9]/g, '').replace(/^0/, '20')}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className={`flex items-center justify-center gap-2 w-full py-2.5 px-4 bg-green-500 hover:bg-green-600 text-white rounded-xl font-medium transition-colors ${isRTL ? 'flex-row-reverse' : ''}`}
                    >
                ...
                {/* Notice about no reply option */}
                <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                  <p className="text-xs text-amber-700">
                    {locale === 'ar'
                      ? 'لا يمكن الرد على هذه الرسالة من خلال المنصة. يرجى التواصل عبر الإيميل أو الواتساب.'
                      : 'Cannot reply to this message via platform. Please contact via email or WhatsApp.'}
                  </p>
                </div>
              </div>
            ) : (
              /* User Info - Show for regular tickets */
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
                <h3
                  className={`font-semibold text-slate-900 mb-3 flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}
                >
                ...
                <div className="space-y-3">
                  ...
                  {ticket.user?.phone && (
                    <div
                      className={`flex items-center gap-2 text-sm text-slate-600 ${isRTL ? 'flex-row-reverse' : ''}`}
                    >
                      <Phone className="w-4 h-4" />
