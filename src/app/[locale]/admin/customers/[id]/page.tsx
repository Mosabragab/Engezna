...
export default function AdminCustomerDetailsPage() {
  ...
  async function loadCustomer(supabase: ReturnType<typeof createClient>) {
    ...
    if (addressData) {
      default_address = {
        governorate: locale === 'ar' ? addressData.governorate?.name_ar : addressData.governorate?.name_en,
        ...
        <main className="flex-1 p-4 lg:p-6 overflow-auto">
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500">
                    {customer.phone && (
                      <div className="flex items-center gap-1">
                        ...
                        <a href={`tel:${customer.phone}`} className="hover:text-red-600">{customer.phone}</a>
                      </div>
                    )}
                    {customer.email && (
                      <div className="flex items-center gap-1">
                        <Mail className="w-4 h-4" />
                        <a href={`mailto:${customer.email}`} className="hover:text-red-600">{customer.email}</a>
                      </div>
                    )}
                    <div className="flex items-center gap-1">
                      <Calendar className="w-4 h-4" />
                      <span>{locale === 'ar' ? 'انضم' : 'Joined'} {formatDate(customer.created_at, locale)}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
            <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
              ...
              <p className="text-2xl font-bold text-slate-900">{formatNumber(customer.orders_count, locale)}</p>
            </div>
            <div className="bg-green-50 rounded-xl p-4 border border-green-200">
              <div className="flex items-center gap-3 mb-2">
                <CheckCircle2 className="w-5 h-5 text-green-600" />
                <span className="text-sm text-green-700">{locale === 'ar' ? 'مكتملة' : 'Completed'}</span>
              </div>
              <p className="text-2xl font-bold text-green-700">{formatNumber(customer.completed_orders, locale)}</p>
            </div>
            <div className="bg-red-50 rounded-xl p-4 border border-red-200">
              <div className="flex items-center gap-3 mb-2">
                <XCircle className="w-5 h-5 text-red-600" />
                <span className="text-sm text-red-700">{locale === 'ar' ? 'ملغاة' : 'Cancelled'}</span>
              </div>
              <p className="text-2xl font-bold text-red-700">{formatNumber(customer.cancelled_orders, locale)}</p>
            </div>
            ...
            <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-sm text-blue-700">{locale === 'ar' ? 'متوسط الطلب' : 'Avg Order'}</span>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-slate-500">
                              {order.provider ? (locale === 'ar' ? order.provider.name_ar : order.provider.name_en) : '-'}
                            </p>
                            <p className="text-xs text-slate-400">{formatDateTime(order.created_at, locale)}</p>
                          </div>
                          <div className="flex items-center gap-4">
            ...
            <div className="space-y-6">
                    ...
                    <div className="space-y-2 text-sm">
                      {customer.default_address.governorate && (
                        <p className="text-slate-600">
                          <span className="text-slate-400">{locale === 'ar' ? 'المحافظة:' : 'Governorate:'}</span>{' '}
                          {customer.default_address.governorate}
                        </p>
                      )}
                      {customer.default_address.city && (
                        <p className="text-slate-600">
                          <span className="text-slate-400">{locale === 'ar' ? 'المدينة:' : 'City:'}</span>{' '}
                          {customer.default_address.city}
                        </p>
                      )}
                      {customer.default_address.district && (
                        <p className="text-slate-600">
                          <span className="text-slate-400">{locale === 'ar' ? 'الحي:' : 'District:'}</span>{' '}
                          {customer.default_address.district}
                        </p>
                      )}
                      {customer.default_address.street && (
                        <p className="text-slate-600">
                          <span className="text-slate-400">{locale === 'ar' ? 'الشارع:' : 'Street:'}</span>{' '}
                          {customer.default_address.street}
                        </p>
                      )}
                    </div>
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div className="p-4 space-y-3">
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-500">{locale === 'ar' ? 'تاريخ الانضمام' : 'Joined'}</span>
                    <span className="text-slate-900">{formatDate(customer.created_at, locale)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-500">{locale === 'ar' ? 'آخر طلب' : 'Last Order'}</span>
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-500">{locale === 'ar' ? 'التحقق' : 'Verification'}</span>
                    {customer.is_verified ? (
                      <span className="text-green-600 flex items-center gap-1">
                        <CheckCircle2 className="w-3 h-3" />
                        {locale === 'ar' ? 'موثق' : 'Verified'}
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div className="p-4 space-y-3">
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-500">{locale === 'ar' ? 'معدل الإكمال' : 'Completion Rate'}</span>
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-500">{locale === 'ar' ? 'معدل الإلغاء' : 'Cancellation Rate'}</span>
                    <span className="text-slate-900 font-medium">
                      {customer.orders_count > 0
