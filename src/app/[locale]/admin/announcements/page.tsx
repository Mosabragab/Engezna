...
const typeConfig: Record<
  AnnouncementType,
  { label: { ar: string; en: string }; color: string; bgColor: string }
> = {
  urgent: {
...
export default function AdminAnnouncementsPage() {
  ...
  async function markAsRead(announcement: Announcement) {
    ...
    await supabase.from('announcements').update({ read_by: newReadBy }).eq('id', announcement.id);

    setAnnouncements((prev) =>
      prev.map((a) => (a.id === announcement.id ? { ...a, read_by: newReadBy } : a))
    );
  }
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-amber-50 rounded-xl p-4 border border-amber-200">
            ...
            <p className="text-2xl font-bold text-amber-700">
              {formatNumber(stats.pinned, locale)}
            </p>
          </div>
                  ...
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded text-xs font-medium ${typeConfig[announcement.type]?.color} ${typeConfig[announcement.type]?.bgColor}`}
                          >
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-4 text-sm text-slate-500">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span>
                          {readCount} {locale === 'ar' ? 'شاهدوا' : 'viewed'}
                        </span>
                      </div>
                      {announcement.expires_at && (
                        <div className="flex items-center gap-2">
                          ...
                          <span>
                            ...
                            {formatDate(announcement.expires_at, locale)}
                          </span>
                        </div>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full p-6 my-8">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <span
                  className={`px-2 py-0.5 rounded text-xs font-medium ${typeConfig[selectedAnnouncement.type]?.color} ${typeConfig[selectedAnnouncement.type]?.bgColor}`}
                >
            ...
            <div className="flex items-center gap-4 text-sm text-slate-500 border-t border-slate-200 pt-4">
              {selectedAnnouncement.creator && (
                <span>
                  {locale === 'ar' ? 'بواسطة:' : 'By:'} {selectedAnnouncement.creator.full_name}
                </span>
              )}
