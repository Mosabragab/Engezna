...
export default function AdminDashboard() {
  ...
  const loadDashboardData = useCallback(
    async (supabase: ReturnType<typeof createClient>, adminData: AdminUser | null) => {
      try {
        // Determine governorate IDs for filtering (only for non-super_admin)
        const isSuperAdmin = adminData?.role === 'super_admin';
        const assignedGovernorateIds =
          !isSuperAdmin && adminData?.assigned_regions
        ...
        const statsResponse = await fetch('/api/admin/stats', {
          ...
          body: JSON.stringify({
            action: 'dashboard',
            filters:
              assignedGovernorateIds.length > 0 ? { governorateIds: assignedGovernorateIds } : {},
          }),
        });
        ...
        let pendingProvidersQuery = supabase
          .from('providers')
        ...
        if (assignedGovernorateIds.length > 0) {
          pendingProvidersQuery = pendingProvidersQuery.in(
            'governorate_id',
            assignedGovernorateIds
          );
        ...
        const [{ data: pendingProvidersData }, { data: recentOrdersData }] = await Promise.all([
        ...
        const transformedOrders = (recentOrdersData || []).map(
          (order: {
            id: string;
            order_number: string;
            total: number;
        ...
        setDataError(
          locale === 'ar' ? 'خطأ في تحميل بيانات لوحة التحكم' : 'Error loading dashboard data'
        );
      }
    },
    [locale]
      ...
      <main className="flex-1 p-4 lg:p-6 overflow-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-slate-500">
            {locale === 'ar'
              ? 'إليك نظرة عامة على منصة إنجزنا'
              : "Here's an overview of Engezna platform"}
          </p>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-elegant-sm card-hover">
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                className={`text-xs flex items-center gap-1 font-numbers ${stats.gmvChange >= 0 ? 'text-green-600' : 'text-red-600'}`}
              >
                {stats.gmvChange >= 0 ? (
            ...
            <p className="text-2xl font-bold text-slate-900 font-numbers">
              {formatCurrency(stats.gmvMonth, locale)}
            </p>
            <p className="text-xs text-slate-500">
              {locale === 'ar' ? 'إجمالي المبيعات (الشهر)' : 'GMV (Month)'}
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-elegant-sm card-hover">
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                className={`text-xs flex items-center gap-1 font-numbers ${stats.ordersChange >= 0 ? 'text-green-600' : 'text-red-600'}`}
              >
            ...
            <p className="text-2xl font-bold text-slate-900 font-numbers">
              {formatNumber(stats.ordersToday, locale)}
            </p>
            <p className="text-xs text-slate-500">
          ...
          <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-elegant-sm card-hover">
            ...
            <p className="text-2xl font-bold text-slate-900 font-numbers">
              {formatNumber(stats.activeProviders, locale)}
            </p>
            <p className="text-xs text-slate-500">
              {locale === 'ar' ? 'المتاجر النشطة' : 'Active Providers'}
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-elegant-sm card-hover">
            ...
            <p className="text-2xl font-bold text-slate-900 font-numbers">
              +{formatNumber(stats.newCustomersToday, locale)}
            </p>
            <p className="text-xs text-slate-500">
              {locale === 'ar' ? 'عملاء جدد اليوم' : 'New Customers Today'}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <Link
            ...
            className="bg-card-bg-amber rounded-xl p-4 border border-amber/30 hover:border-amber/50 transition-all duration-200 hover:-translate-y-1 hover:shadow-lg"
          >
            ...
            <p className="text-3xl font-bold text-amber font-numbers">
              {formatNumber(stats.pendingProviders, locale)}
            </p>
            <p className="text-xs text-amber/80">
              {locale === 'ar' ? 'تنتظر الموافقة' : 'Awaiting Approval'}
            </p>
          </Link>

          {/* Open Disputes - Resolution Center */}
          <Link
            href={`/${locale}/admin/resolution-center`}
            className="bg-card-bg-primary rounded-xl p-4 border border-primary/30 hover:border-primary/50 transition-all duration-200 hover:-translate-y-1 hover:shadow-lg"
          >
            <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-3xl font-bold text-primary font-numbers">
              {formatNumber(stats.openTickets, locale)}
            </p>
            <p className="text-xs text-primary/80">
              {locale === 'ar' ? 'تذاكر + مرتجعات' : 'Tickets + Refunds'}
            </p>
          </Link>
          ...
          <Link
            ...
            className="bg-card-bg-purple rounded-xl p-4 border border-purple/30 hover:border-purple/50 transition-all duration-200 hover:-translate-y-1 hover:shadow-lg"
          >
            <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-3xl font-bold text-purple font-numbers">
              {formatCurrency(stats.pendingSettlements, locale)}
            </p>
          ...
          <div className="bg-card-bg-success rounded-xl p-4 border border-success/30 card-hover">
            ...
            <p className="text-3xl font-bold text-success font-numbers">
              {formatCurrency(stats.commissionsMonth, locale)}
            </p>
                ...
                recentOrders.map((order) => (
                  <div key={order.id} className="p-4 hover:bg-slate-50 transition-colors">
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      {formatTimeAgo(order.created_at, locale)}
                    </p>
                  </div>
                ))
              ) : (
              ...
              {pendingProviders.length > 0 ? (
                pendingProviders.map((provider) => (
                  <div key={provider.id} className="p-4 hover:bg-slate-50 transition-colors">
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      {formatTimeAgo(provider.created_at, locale)}
                    </p>
                  </div>
                ))
              ) : (
        ...
        <div className="mt-6 bg-white rounded-xl p-6 border border-slate-200 shadow-elegant-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            ...
            <div>
              <p className="text-sm text-slate-500 mb-1">
                {locale === 'ar' ? 'طلبات الأسبوع' : 'Weekly Orders'}
              </p>
              <p className="text-2xl font-bold text-slate-900 font-numbers">
                {formatNumber(stats.ordersWeek, locale)}
              </p>
            </div>
            <div>
              <p className="text-sm text-slate-500 mb-1">
            ...
            <div>
              <p className="text-sm text-slate-500 mb-1">
                {locale === 'ar' ? 'العمولات (الشهر)' : 'Commissions (Month)'}
              </p>
              <p className="text-2xl font-bold text-slate-900 font-numbers">
                {formatCurrency(stats.commissionsMonth, locale)} {locale === 'ar' ? 'ج.م' : 'EGP'}
              </p>
            </div>
          </div>
        </div>
