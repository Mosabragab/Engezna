    ...
    try {
      ...
      if (paymentMethod === 'cash') {
        ...
        if (appliedPromoCode) {
          ...
          if (usageError) {
            ...
            await supabase
              ...
              .eq('id', appliedPromoCode.id);
            throw new Error(
              locale === 'ar' ? 'فشل في تسجيل كود الخصم.' : 'Failed to record promo code usage.'
            );
          }
        ...
        if (orderError) {
          ...
          // Rollback promo usage on order creation failure
          if (appliedPromoCode) {
            if (promoUsageId) {
        ...
        // Link the promo usage record to the created order
