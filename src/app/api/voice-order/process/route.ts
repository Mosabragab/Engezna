      ...
      parameters: {
        ...
        properties: {
          query: {
            type: 'string',
            description:
              'The item name or description to search for (e.g., shawarma, coffee, burger)',
          },
          providerId: {
            type: 'string',
...
async function searchProviders(query: string, category?: string) {
  ...
  let dbQuery = supabase
    .from('providers')
    .select(
      'id, name_ar, name_en, category, rating, delivery_fee, estimated_delivery_time_min, status'
    )
...
async function searchMenuItems(query: string, providerId?: string) {
  ...
  dbQuery = dbQuery.or(
    `name_ar.ilike.%${query}%,name_en.ilike.%${query}%,description_ar.ilike.%${query}%,description_en.ilike.%${query}%`
  );
...
async function processFunctionCall(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
...
1. البحث عن المتاجر المحلية (مطاعم، سوبر ماركت، بقالة، صيدليات، وغيرها)
