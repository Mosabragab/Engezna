...
export const POST = withErrorHandler(async (request: NextRequest) => {
  ...
  if (!rateLimitResult.success) {
    return rateLimitErrorResponse(rateLimitResult) as unknown as NextResponse;
  }
  ...
  const cartTotal =
    body.cart_total ||
    body.cart_items?.reduce((sum, item) => sum + item.unit_price * item.quantity, 0) ||
    0;

  // Fetch customer memory for personalization (only for logged-in users)
  const customerMemory = body.customer_id ? await getCustomerMemory(body.customer_id) : null;
  ...
  const context: AgentContext = {
    ...
    providerContext:
      body.selected_provider_id && body.selected_provider_name
            ...
            case 'done':
              ...
              controller.enqueue(
                sse.encode('message', {
                  ...
                  cart_actions:
                    processedCartActions && processedCartActions.length > 0
