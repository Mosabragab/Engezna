...
export function useAIChat(options: UseAIChatOptions = {}): UseAIChatReturn {
  ...
  const {
    ...
    removeItemCompletely: cartRemoveItemCompletely,
    ...
    async (message: string) => {
      ...
      try {
          ...
          if (action.type === 'REMOVE_ITEM') {
            ...
            if (itemToRemove) {
              ...
              if (removeQty > 0 && removeQty < currentQty) {
                ...
                cartUpdateQuantity(
                  itemToRemove.menuItem.id,
                  newQty,
                  itemToRemove.selectedVariant?.id
                );
              } else {
                // Remove completely
                cartRemoveItemCompletely(
                  itemToRemove.menuItem.id,
                  itemToRemove.selectedVariant?.id
                );
              }
        ...
        // Supports: ADD_ITEM, CLEAR_AND_ADD, CLEAR_CART, REMOVE_ITEM, UPDATE_QUANTITY
          ...
          data.cart_action &&
          ['ADD_ITEM', 'CLEAR_AND_ADD', 'CLEAR_CART', 'REMOVE_ITEM', 'UPDATE_QUANTITY'].includes(
            data.cart_action.type
          )
        ) {
  ...
  const sendQuickAction = useCallback(
    async (action: string, payload?: string) => {
      ...
      const quickReplies = (lastAssistantMessage as ChatMessage & { quickReplies?: QuickReply[] })
        ?.quickReplies;
      // Check both by title AND by payload
      const matchingReply = quickReplies?.find(
        (qr) => qr.title === action || qr.payload === action
      );
      ...
      if (matchingReply) {
        ...
        const actionMessages: Record<string, string> = {
          ...
          '📋 شوف المنيو': selectedProviderId
            ? `navigate:/ar/providers/${selectedProviderId}`
            : 'مرحبا',
          '🛒 شوف المنتجات': selectedProviderId
            ? `navigate:/ar/providers/${selectedProviderId}`
            : 'مرحبا',
          // Also handle the text payload (when agentHandler didn't have providerId but useAIChat has selectedProviderId)
          'عايز أشوف المنتجات': selectedProviderId
            ? `navigate:/ar/providers/${selectedProviderId}`
            : 'عايز أشوف المنتجات',
          '🔍 ابحث': 'search',
  ...
  const addToCartFromChat = useCallback(
    (product: ChatProduct, quantity: number = 1) => {
